...
{
    std::string fullName = "gaussian_2d_bgk/" + fName+ ".dat";
...
void collide_bulk()
{
    ...
    for(int iY = 0; iY < NY; ++iY)
        for(int iX = 0; iX < NX; ++iX)
            for (int iPOP = 0; iPOP < NPOP; ++iPOP)
            {
                double geq = weights[iPOP] * phase[iY][iX] * (1.0 + 
                             3.0 * cx[iPOP] * ux_main + 3.0 * cy[iPOP] * uy_main + 
                             4.5 * (cx[iPOP] * cx[iPOP] - 1.0 / 3.0) * ux_main * ux_main +
                             9.0 * cx[iPOP] * cy[iPOP] * ux_main * uy_main +
                             4.5 * (cy[iPOP] * cy[iPOP] - 1.0 / 3.0) * uy_main * uy_main);
                  g2[iY][iX][iPOP] = g[iY][iX][iPOP] - omega * (g[iY][iX][iPOP] - geq);    
              }
}
...
int main(int argc, char* argv[])
{
    ...
    for(int counter = 0; counter <= N; ++counter)
    {
        ...
        if (counter%NOUTPUT==0)
        {
            ...
            filewritephase<<"gaussian_2d_bgk"<<std::string(6-counterconvert.str().size(),'0')<<counter;
