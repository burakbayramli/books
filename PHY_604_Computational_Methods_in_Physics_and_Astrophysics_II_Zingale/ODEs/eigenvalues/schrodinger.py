...
class Schrodinger(object):
    ...
    def __init__(self, V, x_match=0.5, x_far=10, nsteps=100, tol=1.e-6):
        ...
        self.xi_all = np.concatenate((self.xi_left, self.xi_right[::-1]))
    ...
    def match(self, E, plot_intermediate=False):
        ...
        # boundary conditions [psi, phi] -- note: the sign in the
        # _left solution comes from the fact that x_far is defined as
        # positive
        ...
        # for looking at the solution convergence
        if plot_intermediate:
            Vs = self.V(self.xi_all)
            plt.clf()
            plt.plot(self.xi_all, Vs/max(np.abs(Vs)), lw=2, label=r"$V(x)/\max{V(x)}$")

            max_psi = max(psi_l.max(), psi_r.max())
            plt.plot(xi_l, psi_l/max_psi)
            plt.plot(xi_r, psi_r/max_psi)
            plt.title("E = {}".format(E))

            plt.savefig("match_{:04d}.png".format(self.niter))

        return yl[1]/yl[0] - yr[1]/yr[0], xi, psi

    def solve(self, E1, E2, plot_intermediate=False):
        ...
        m_old, _, _ = self.match(E1, plot_intermediate=plot_intermediate)
        self.niter += 1

        while np.abs(dE) > self.tol:
            m_new, xi_s, psi_s = self.match(E2, plot_intermediate=plot_intermediate)
