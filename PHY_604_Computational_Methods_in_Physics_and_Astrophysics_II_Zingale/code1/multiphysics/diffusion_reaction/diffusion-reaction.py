...
class Grid(object):
    ...
    def initialize(self, L=20, delta=1):
        """ initial conditions """
        ...
        phi[:] = 0.0
        phi[:] = 0.5*(1.0 + np.tanh((self.x-x1)/delta)) * \
                 0.5*(1.0 - np.tanh((self.x-x2)/delta))
...
plt.xlabel("$x$")
plt.ylabel("\phi$")
plt.title(r"Diffusion-Reaction, $N = {}, \, \kappa = {:3.2f}, \, \tau = {:3.2f}$".format(nx, kappa, tau))
