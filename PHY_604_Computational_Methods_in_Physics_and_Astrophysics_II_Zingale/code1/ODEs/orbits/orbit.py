...
class OrbitHistory(object):
    ...
    def energy(self):
        """ return energy / mass """
        return 0.5*(self.u**2 + self.v**2) - GM/np.sqrt(self.x**2 + self.y**2)
...
class Orbit(object):
    ...
    def int_Euler_Cromer(self, dt, tmax):
        ...
        while t < tmax:
            ...
            # get the RHS
            _, _, udot, vdot = self.rhs([x, y], [u, v])
    ...
    def int_VVerlet(self, dt, tmax):
        ...
        # get the RHS
        _, _, udot_old, vdot_old = self.rhs([x, y], [u, v])
        ...
        while t < tmax:
            ...
            xnew = x + dt*u + 0.5*dt**2*udot_old
            ynew = y + dt*v + 0.5*dt**2*vdot_old

            _, _, udot, vdot = self.rhs([xnew, ynew], [u, v])

            unew = u + 0.5*dt*(udot_old + udot)
            vnew = v + 0.5*dt*(vdot_old + vdot)
