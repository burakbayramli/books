...
def mkMatrix(dt):
    """ return I - dt A """
    return np.matrix([[1.0 + dt*alpha, -dt*beta     ],
                      [-dt*alpha,      1.0 + dt*beta]])

def rhs(t, Y):
    """ RHS of the system for explicit methods """
    return np.array([-alpha*Y[0] + beta*Y[1], alpha*Y[0] - beta*Y[1]])
...
def trapIntegrate(YA_0, YB_0, dt, tmax):
    ...
    while (t < tmax):
        ...
        b = np.array([(1.0 - 0.5*dt*alpha)*YA + 0.5*dt*beta*YB,
...
plt.savefig("stiff-linear-dt0.5tref.png")
...
plt.xlabel(r"$t \cdot (\alpha + \beta)$")
plt.ylabel(r"$(Y_B/Y_A) / (\alpha/\beta)$")
plt.title(r"$\Delta t = 5 / (\alpha + \beta)$")

ax = plt.gca()
ax.xaxis.set_major_formatter(plt.ScalarFormatter(useMathText=True))
ax.yaxis.set_major_formatter(plt.ScalarFormatter(useMathText=True))

plt.savefig("stiff-linear-dt5tref.png")
...
plt.savefig("stiff-linear-dt5tref-VODE.png")
