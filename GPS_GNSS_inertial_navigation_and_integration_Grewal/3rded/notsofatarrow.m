function result = notsofatarrow(tail,tip,cuv)
%
...
w = w/norm(w);
%
% Shaft length equals eighty percent of arrow length
%
...
patch(se(1)+cr*p(1,:),se(2)+cr*p(2,:),se(3)+cr*p(3,:),esh);
patch(tail(1)+sr*p(1,:),tail(2)+sr*p(2,:),tail(3)+sr*p(3,:),esh);
%
...
for k=1:72,
    ...
    sh = abs(sn'*cuv); % Cone shading coefficient
    sh = sh*[1,1,1];
    %
    % Cone patch
    %
    patch([se(1)+cr*p(1,k),se(1)+cr*p(1,k+1),tip(1)],[se(2)+cr*p(2,k),se(2)+cr*p(2,k+1),tip(2)],[se(3)+cr*p(3,k),se(3)+cr*p(3,k+1),tip(3)],sh,'LineStyle','none');
    %
    % Shaft patch
    %
    patch([se(1)+sr*p(1,k),se(1)+sr*p(1,k+1),tail(1)+sr*p(1,k+1),tail(1)+sr*p(1,k)],[se(2)+sr*p(2,k),se(2)+sr*p(2,k+1),tail(2)+sr*p(2,k+1),tail(2)+sr*p(2,k)],[se(3)+sr*p(3,k),se(3)+sr*p(3,k+1),tail(3)+sr*p(3,k+1),tail(3)+sr*p(3,k)],ss(k)*[1,1,1],'LineStyle','none');
end;
result = 1;
return;
%%  
