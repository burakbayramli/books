...
%
   ...
   el(i)	=	atan(ENU(3)/(ENU(1)^2+ENU(2)^2)^.5)*180./pi;
   az(i) = 	atan(ENU(1)/ENU(2))*180./pi;
   if(ENU(2) <= 0.)
      az(i) = az(i) + 180.;
   end
   az(i) = mod(az(i), 360);
...
%figure;
%hold on;
%title (['Satellite Position in ECEF Coordinate, PRN = ', num2str(n_prn)],'FontSize',8,'HorizontalAlignment','Center'); 
%subplot(2,2,1);
%plot(t_plot,xk);
%axis([0 24 -3e7 3e7]);
%xlabel('hours');
%title (['x in meters, PRN = ', num2str(n_prn)],'FontSize',8,'HorizontalAlignment','Center');
%subplot(2,2,2);
%plot(t_plot,yk);
%axis([0 24 -3e7 3e7]);
%xlabel('hours');
%title (['y in meters, PRN = ', num2str(n_prn)],'FontSize',8,'HorizontalAlignment','Center');
%subplot(2,2,3);
%plot(t_plot,zk);
%axis([0 24 -3e7 3e7]);
%xlabel('hours');
%title (['z in meters, PRN = ', num2str(n_prn)],'FontSize',8,'HorizontalAlignment','Center');
%plot3(xk,yk,zk);
end
fclose(fid);
%
figure;
plot3(x_sv,y_sv,z_sv);
title ('Satellite Position in ECEF Coordinate ','FontSize',8,'HorizontalAlignment','Center'); 
grid on;
axis equal;
%view([45 45]);
%y = el.*sin(az-90.)*pi/180.);
%x = el.*cos(az-90.)*pi/180.;
%h = oplar([0 2*pi),[0 90]);
%delete(h);
%hold on;
%plot(x,y);  
