...
[tauv,rhovelacc,sigmajerk] = Damp2Params(sigmav,sigmaa,taua,Dt);
%
...
subplot(2,1,1),
plot(Time(10:Nsecs)/3600,RMSDiff,'k-');
text(2,6,['{\sigma}_{Kalman} = ',num2str(RMSKal),' [m]']);
title('Difference between Schmidt-Kalman Filter and Kalman Filter RMS Error/Axis');
xlabel('Time [hr]');
ylabel('Difference [m]');
subplot(2,1,2),
plot(9/3600,0,'w.',NoSecs/3600,12,'w.',Time(10:Nsecs)/3600,NoSatsInView(10:Nsecs),'k-');
%title([num2str(NoSecs/60/60),' Hrs of Simulated GPS Operation']);
xlabel('Time [hr]');
ylabel('No. Satellites Used');
