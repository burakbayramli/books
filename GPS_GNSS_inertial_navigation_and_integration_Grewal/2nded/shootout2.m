...
loglog(d,rdJ,'k-',d,rdP,'k-',d,rdC,'k-',d,rdB,'k-',d,rdK,'k-',d,rdS,'k-');
ylabel('RMS Relative Error in P');
xlabel('Conditioning variable \delta');
...
text(eps,bottom,'!');
text(eps/3,10*bottom,texlabel(['log_2(epsilon) = ',num2str(log2(eps))]));
text(sqrt(eps),bottom,'!');
text(sqrt(eps)/3,10*bottom,texlabel('sqrt(epsilon)'));
%
