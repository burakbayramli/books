      program lw 
c...Performs Lax-Wendroff's method
      parameter (nmax=600,delta=.000001)
      real*8 lambda, u(-2:nmax+2), h(0:nmax)
      real*8 u0(1:nmax+1),a(0:nmax), xmin, s1, s2
      real*8 xmax, dummy, tfinal, delta_x, delta_t
      real*8 f, df, x
...
c      do 100, i=0,n
c        if(abs(u(i+1)-u(i)).gt.delta) then
c         a(i) = (f(u(i+1))-f(u(i)))/(u(i+1)-u(i))
c        else
c         a(i) = df(u(i))
c        endif
c 100  continue
...
c      do 110, i=0,n
c        h(i) = f(u(i+1))+f(u(i)) - lambda*(u(i+1)-u(i))*(a(i)**2)
c 110  continue

c      do 120, i=1,n
c        u(i) = u(i) -.5*lambda*(h(i) - h(i-1))
c 120  continue
...
c...For the LAE only
      do 100, i=1,n
        s1 = lambda
        s2 = lambda**2
        h(i) = .5*(s2-s1)*u(i+1)+(1.-s2)*u(i)+.5*(s2+s1)*u(i-1)
 100  continue
...
C...write simple file for plotting
 ...
 1050 format(4x,'N',11x,'INITIAL',12x,'L-W',11x, 'DIFFERENCE')
