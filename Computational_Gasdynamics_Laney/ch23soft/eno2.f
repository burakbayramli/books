...
c       t1 = mm(delta2_u(i),delta2_u(i+1))
c       t2 = mm(delta2_u(i-1),delta2_u(i))
c       S(i) = minmod(delta_u(i)-.5*t1,delta_u(i-1)+.5*t2)
...
c...    S(i) = delta_u(i2) + (real(i-i2)-.5)*mm(delta2_u(i2+1),delta2_u(i2))
...
c     write(10,*)  n, lambda, tfinal
 ...
 1050 format(4x,'N',11x,'INITIAL',11x,'ENO-2',10x, 'DIFFERENCE')
