...
s = Matrix([[a*x1**2+d*x1+3*x2+x3,-b*x2*x1,0],
            [-b*x2*x1, 5*x2**2,0],
            [0,0,c*x3**2]])
print("stress field inside a body in static equilibrium")
print("\u03C3 =", s)
pb = Matrix([x1, x2, x3])*10
print("body force vector")
print("pb =", pb)
# systems of equation:
...
print("system of equations =",Eq(equation[0],0),Eq(equation[1],0),Eq(equation[2],0))
Eq0 = Eq(d,0)
Eq1 = Eq(equation[0].coeff(x1),0)
Eq2 = Eq(equation[1].coeff(x2),0)
Eq3 = Eq(equation[2].coeff(x3),0)
print("because these equations are valid for all values of x1, x2, and x3. They can be refined to =",Eq0,Eq1,Eq2,Eq3)
