function [du] = KPPCrhs2D(x,y,u,hx,hy,k,maxvel)
% function [du] = KPPCrhs2D(x,y,u,hx,hy,k,maxvel);
% Purpose: Evaluate right hand side for 2D KPP equation using second
% order central scheme
Nxy=size(x); Nx=Nxy(2); Ny=Nxy(1); Nxs=Nx-1; Nys=Ny-1; du=zeros(Ny,Nx); kl = k/2;
uG = zeros(Ny+4,Nx+4); duLx = zeros(Ny+2,Nx+2); duLy = zeros(Ny+2,Nx+2);
uGs = zeros(Nys+4,Nxs+4); duLxs = zeros(Nys+2,Nxs+2); duLys = zeros(Nys+2,Nxs+2);
...
for i=1:Ny
   [xe,ue] = extend(x(1,:),u(i,:),hx,2,'D',pi/4,'D',pi/4); 
   uG(i+2,:) = ue';
end
for j=1:Nx+4
   [xe,ue] = extend(y(:,1),uG(3:Ny+2,j),hy,2,'D',pi/4,'D',pi/4); 
   uG(:,j) = ue;
end
...
uhG = uloc - kl/2.*(KPPJac2Dx(uloc).*duLx/hx + KPPJac2Dy(uloc).*duLy/hy);
...
for i=1:Ny-1
  ...
  Flux1 = (ue(3:Nx+1)+ue(4:Nx+2))/4 + (duL(2:Nx)-duL(3:Nx+1))/8 ...
      - kl/hx*(KPPFlux2Dx(uh(3:Nx+1))-KPPFlux2Dx(uh(2:Nx)));
  % Right term
...
for j=1:Nx-1
  ...
  Flux1 = (ue(3:Ny+1)+ue(4:Ny+2))/4 + (duL(2:Ny)-duL(3:Ny+1))/8 ...
      - kl/hy*(KPPFlux2Dy(uh(3:Ny+1))-KPPFlux2Dy(uh(2:Ny)));
  % Up term
...
% Extend data and assign boundary conditions on global data
for i=1:Nys
   [xe,ue] = extendstag(x(1,1:Nxs),us(i,:),hx,2,'D',pi/4,'D',pi/4); 
   uGs(i+2,:) = ue';
end
for j=1:Nxs+4
   [xe,ue] = extendstag(y(1:Nys,1),uGs(3:Nys+2,j),hy,2,'D',pi/4,'D',pi/4); 
   uGs(:,j) = ue;
end
...
uhGs = uloc - kl/2.*(KPPJac2Dx(uloc).*duLxs/hx ...
                 + KPPJac2Dy(uloc).*duLys/hy);
...
for i=1:Nys+1
  ...
  Flux1 = (ue(2:Nxs+2)+ue(3:Nxs+3))/4 + (duL(1:Nxs+1)-duL(2:Nxs+2))/8 ...
      - kl/hx*(KPPFlux2Dx(uh(2:Nxs+2))-KPPFlux2Dx(uh(1:Nxs+1)));
  % Right term
...
for j=1:Nxs+1
  ...
  Flux1 = (ue(2:Nys+2)+ue(3:Nys+3))/4 + (duL(1:Nys+1)-duL(2:Nys+2))/8 ...
      - kl/hy*(KPPFlux2Dy(uh(2:Nys+2))-KPPFlux2Dy(uh(1:Nys+1)));
  % Up term
