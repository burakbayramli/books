...
% same as demo4.m but with specified heat radiation on surface of water
...
maxiter = 200;   % step number for time iteration, 
DT      = 0.1;
...
if Start == 1, 
   ...
   % -- Boundary conditions --------------------
   [RDU,RDV,RDT,RDP,FU,FV,FT,RDZ,RCZ,RCT] = feval(FF3,p,e,p1);
   ...
   INVM  = DT*INVM;
   save daten5a p e t p1 t1
   save daten5c S INVM GRADP MATRIXP RD MM2 RCT RDP
end  
load daten5a p e t p1 t1
load daten5c S INVM GRADP MATRIXP RD MM2 RCT RDP
...
for I = 1:maxiter  
   ...
   RSIDEP(RDP(1)) = RDP(2);
   P1 = GRADP\RSIDEP;
   B = radiation_bc(p,p1,UU1(2*N+1:3*N),RCT,DT,kappa,t_air);
   FF1 = -A*UU1 + MATRIXP*P1 + B; 
   ...
   RSIDEP(RDP(1)) = RDP(2);
   P2 = GRADP\RSIDEP; 
   B = radiation_bc(p,p1,UU2(2*N+1:3*N),RCT,DT,kappa,t_air);
   FF2 = -A*UU2 + MATRIXP*P2 + B;
   ...
   RSIDEP(RDP(1)) = RDP(2);
   P3 = GRADP\RSIDEP; 
   B = radiation_bc(p,p1,UU3(2*N+1:3*N),RCT,DT,kappa,t_air);
   FF3 = -A*UU3 + MATRIXP*P3 + B;
...
save daten5b U V TT P 
disp(' Call bild01 and select demo5 ! ')
