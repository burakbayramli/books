function [AU,BU,AV,BV,AT,BT] = triform1_aux(X,Y,U,V,T)
...
AV = F(1,:)*V*M + F(2,:)*V*P + F(3,:)*V*Q;
BV = G(1,:)*V*M + G(2,:)*V*P + G(3,:)*V*Q;
AT = F(1,:)*T*M + F(2,:)*T*P + F(3,:)*T*Q;
BT = G(1,:)*T*M + G(2,:)*T*P + G(3,:)*T*Q;
