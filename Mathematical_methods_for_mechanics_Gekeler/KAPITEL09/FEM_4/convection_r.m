function TN = convection_r(p,t,Z,RDT,RCT,T,Parmeter,DELTA_T,CONVEC_T)
DT = Parmeter(1);
LAMDA = Parmeter(3); KAPPA = Parmeter(5); TAIR = Parmeter(6);
...
for I = 1:M
   ...
   %%% AENDERUNG %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   B_AUX = B_AUX + DT*MM*(- DELTA_T(K) + LAMDA*CONVEC_T(K));
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   A(K,K) = A(K,K) + A_AUX;
   B(K)   = B(K)   + B_AUX;
end
...
if ~isempty(RCT)
   for I = 1:size(RCT,2)
      ...
      B(K)    = B(K) - DT*KAPPA*(ME*T(K) - TAIR*BE);
   end
end
% -- Dirichlet boundary condition of temperature --
J      = RDT(1,:);
B      = B - A(:,J)*RDT(2,:)';
B(J)   = RDT(2,:)';
A(J,:) = 0; A(:,J) = 0;
