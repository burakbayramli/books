function Y = rside11(T,XX,Parmeter,p,e,t,FF2,MASSMATRIX);

% WIE RSIDE10, aber Modification fuer Z mit Cauchy-RB
...
% -- Randwerte Einfuegen ---------------
[RDU,RDV,RDZ,RCZ] = feval(FF2,e,T,Parmeter);
...
for I = 1:M
   ...
   SV   = - MM*AUXV - g6*D*Z(K);
   % -- Z part: int(dz_x*w*u + dz_y*w*v) -- modified by partial int.
   AUXZ = W(K).'*MM*U(K)*(C.') + W(K).'*MM*V(K)*(D.');
   SZ   = AUXZ;
   % -- Summieren -------------------------
...
for I = 1:size(RCZ,2)
   K = RCZ(1:2,I);
   [ME,BE] = fem_ralell(p(1,K),p(2,K));
   RSZ(K) = RSZ(K) + 0.5*sum(U(K))*ME*W(K);
end
Y = [RSU;RSV;RSZ];
if nargin == 8
   % all three equations are multiplied with inverse
   % of mass matrix
   BB = [RSU,RSV,RSZ]; % notwendig!
   AUX = MASSMATRIX\BB;
