function Y = rside_1(XX,p,e,t,Parmeter,FF2,T)
...
[RDU,RDV,RDZ,RCZ] = feval(FF2,e,T,A,PERIOD);
for I = 1:size(RCZ,2)
  K = RCZ(1:2,I);
  X1 = p(1,K(1)); X2 = p(1,K(2));
  Y1 = p(2,K(1)); Y2 = p(2,K(2));
  X21 = X(2) - X(1); Y21 = Y(2) - Y(1);
  L   = sqrt(X21*X21 + Y21*Y21);
  RSZ(K) = RSZ(K) - L*W(K)*ME*(U(K(1)) + U(K(2)))/2;
end
Y = [RSU;RSV;RSZ];
