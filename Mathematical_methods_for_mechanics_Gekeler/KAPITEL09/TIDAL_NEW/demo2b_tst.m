function demo2b_tst
% Ergebnisse schlechter als demo2b.m
...
case 2, disp(' Channel length 1000 m, diverse periods')
   FF1 = 'bsp02'; FF2 = 'bsp02h'; 
   XSCALE  = 25; YSCALE  = 10; L = 1000; H = 20;
...
switch Start
case 1
   V = zeros(3,N);  % Cold start, all bc zero 
   T = 0;
...
for ITER = 1:MAXITER
   ...
   VAUX(1,RDU(1,:)) = RDU(2,:); VAUX(2,RDV(1,:)) = RDV(2,:);
   VAUX(3,RDZ(1,:)) = RDZ(2,:);
  VN = V + (0.5*DT*rside_1(V,p,e,t,Parmeter))*INVLUMP; % vers. 1, good
 % VN = (V*MM + 0.5*DT*rside_1(V,p,e,t,Parmeter))*INVLUMP; % vers. 3, wrong
  % VN = (V*MME + 0.5*DT*rside_1(VAUX,p,e,t,Parmeter,FF2,T))*INVLUMP;  %vers.4, good
  ...
  % VN = (V*MME + DT*rside_1(VN,p,e,t,Parmeter))*INVLUMP;  % good
   ...
   MAXITER_ITER_MAXZ = [MAXITER,ITER,abs(V(3,7))]
end
switch Example
