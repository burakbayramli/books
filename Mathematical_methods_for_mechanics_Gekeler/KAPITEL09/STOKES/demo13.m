...
disp(' U0 = 4, Sequence for NU: New start:0.5,Restart:0.1,0.05,0.03,0.025')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if Start == 1 
   ...
   save daten13a p e t p1 t1 S C D S_LIN B
end
disp(' Boundary conditions and NU may be changed with restart only ')
load daten13a p e t p1 t1 B S C D S_LIN B
N1 = size(p,2); N2 = size(p1,2); N = N1 + N2;
A = [nu*S      , zeros(N,N), -C;
     zeros(N,N), nu*S      , -D;
     C.'        , D.'        , epsilon*S_LIN];
