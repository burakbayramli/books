...
maxiter = 50;   % step number for time iteration, 250 for good result
DT      = 0.05;       % time step  
Parmeter = [nu,DT];
...
if Start == 1, 
   ...
   save daten9a p e t p1 t1 S INVMM AUX GRADP MATRIXP RD J
end  
disp(' NU may be changed without new start ')
load daten9a p e t p1 t1 S INVMM AUX GRADP MATRIXP RD J
N = size(p,2) + size(p1,2); NULL = zeros(N,N);
% -- starting values --------------
