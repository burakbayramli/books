...
% in unit square
demo_number = 100;
while ~ismember(demo_number,[1,4,])
   demo_number = input(' Select demo_number! (1/4) ');
end
%demo_number = 10;
switch demo_number
case 1, load daten1a p e t p1 t1, load daten1b U V P 
case 4, load daten4a p e t p1 t1, load daten4b U V P 
case 6, load daten6a p e t p1 t1, load daten6b U V P 
case 7, load daten7a p e t p1 t1, load daten7b U V P 
case 8, load daten8a p e t p1 t1, load daten8b U V P 
case 9, load daten9a p e t p1 t1, load daten9b U V P 
...
case 1, disp(' Contour of stream function Z*100 ')
   [RDU,RDV,RDP,FU,FV,RDZ,RCZ,RCP] = bsp01h_2(p,e,p1);
%   LASTEN  =  rside_post(p,t,t1,U,V,RCP); % also RCP = []
   ...
   [C,h] = contour(XA,YA,W,[-10,-9,-8,-6,-4,-2,-1,-0.05],'k','linewidth',1); hold on
...
case 6, disp(' Find index of some special points ')
   ...
   load daten12d Z 
   ZWERT = Z(N)
case 7, disp(' contour of differences ')   
   %clf
   xlin    = linspace(min(X),max(X),50);
   ylin    = linspace(min(Y),max(Y),50);
   [XA,YA] = meshgrid(xlin,ylin);
   W = griddata(X,Y,DIFFU(1:N1),XA,YA,'cubic');
  %  W = griddata(X,Y,DIFFV(1:N1),XA,YA,'cubic');
   clf, mesh(XA,YA,W)
  % [C,h] = contour(XA,YA,W,10);
  % grid on
end
clear
