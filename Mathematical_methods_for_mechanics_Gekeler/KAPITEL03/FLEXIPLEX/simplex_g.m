function [XEND,PFAD] = simplex_g(PROBLEM,X1,Parmeter)
% minimizes a function by the method  of Nelder and Mead
% mit Grafik, only two variables
%%%%%%%%%%%%%%%%
X1 = X1';
%%%%%%%%%%%%%%%%%%%%%%%%
...
DONE = 0; PFAD = X1(1,:);
while ~DONE
   plot([X1(1:3,1);X1(1,1)],[X1(1:3,2);X1(1,2)],'b'), hold on
   ...
   circle(X1(INDEX,1),X1(INDEX,2),RR,'b')
   circle(X1(K2,1),X1(K2,2),RR,'r')
   circle(X1(K3,1),X1(K3,2),RR,'g')
   %pause(1)
   if SUM(K3) < SUML
      %disp(' Expansion')
