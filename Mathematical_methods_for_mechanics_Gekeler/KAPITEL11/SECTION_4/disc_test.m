function [M1N,M2N,XN,YN,X1N,Y1N,EINFUEG] ...
         = disc_test(M1,M2,X,Y,X1,Y1,PP,Parmeter)
% E.GEKELER, RELEASE 15.3.04
...
% ----------------------------------
EINFUEG = 0;
ITER = Parmeter(7); MONITOR = 0;
if ITER >= 32, MONITOR = 1; end
MONITOR = 1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if MONITOR == 1
   clf, hold on
   ...
   plot(Y1(1,1:NN2),Y1(2,1:NN2),'.b'), hold on
   plot(PP(1),PP(2),'k*'), hold on
   plot(X1(1,1),X1(2,1),'ro','markersize',12), hold on
   plot(Y1(1,1),Y1(2,1),'b*','markersize',12), hold on
...
if MONITOR == 1, COSA = COS1, COSB = COS2, end
...
if (COS2(K) >= COS1(J) & COS2(K) < MINCOS) | ...
   (COS1(J) >= COS2(K) & COS1(J) < MINCOS) | ...
    (max([COS1,COS2]) <= 0)
%   CS = 0; INDEX1 = 0; INDEX2 = 0;  % DIESEN FALL NICHT WEGLASSEN!
   CS = MINCOS; INDEX1 = 0; INDEX2 = 0;  % DIESEN FALL NICHT WEGLASSEN!
   X1A = X1; Y1A = Y1;
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   if MONITOR == 1, disp('Kein Einfuegen, CS = MINCOS')
   plot(X1(1,:),X1(2,:),'r','linewidth',2), hold on
   plot(Y1(1,1:NN2),Y1(2,1:NN2),'b','linewidth',2), hold on
   pause
   end
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
