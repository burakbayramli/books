function [M1N,M2N,XN,YN,X1N,Y1N] ...
         = disc_rotate(M1,M2,X,Y,X1,Y1,PP,Parmeter)
% E.GEKELER, RELEASE 15.3.04
NN1 = Parmeter(1); NN2 = Parmeter(2); TOL = Parmeter(3);
A_AUF_B = Parmeter(4); OPTION = Parmeter(5); DREHMAX = Parmeter(6);
...
% --Max. Drehgeschwindigkeit Einstellen  ------------------
MAXDREH = 0;
X1 = X1A; Y1 = Y1A;
if MAXDREH == 1
   if CS > DREHMAX              % MAX DREHGESCHWINDIGKEIT EINSTELLEN!
      X1 = X1A; Y1 = Y1A;
   else
      CS = DREHMAX;
   end
