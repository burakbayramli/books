function [t, U] = BackwardEulerNewtonAdap(U0, I, f, TOL, Sc)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% Edit the stepsize parameter k so that it 
% uses the computed stab.factor. Sc !!!


  while(t(i) < T)
    if i > 1
	disp('error - you must change the program!')
        %   k = TOL/(Sc*norm(U(:,i)-U(:,i-1))/k); <---change here to correct Sc
    else
        k=0.01;
    end
    t(i + 1) = t(i) + k;
