function [x, t, U] = fwdtimecentspace(phi, L, A, B, T, N, M, alpha,q) 
% solves the one dimensional heat problem 
% u_t = alpha(t,x,u)*u_xx+q(x,t)
% using the explicit forward time center-space method. 
...
V = zeros(N,1);, V(1)=mu(2)*U(j-1,1);, V(N)=mu(N+1)*U(j-1,N+2);
Q = k*qvec(2:N+1)';

%We now form the next time level approximation.  Notice we have avoided
