...
C
c******************************************************************
c  In the case of unstructured grid arrangement, one has to 
c  assigne to each fine grid CV an integer pointer to the coarse
c  grid CV which contains that fine grid CV (child CV needs to know
c  who is its parent CV). Then, the loop below should run over the
c  list of fine grid CVs, and the value at the center of a fine
c  CV is calculated by calling FICF(IJG,IJF), where IJF is the
c  index of fine grid CV (child) and IJG is the index of the coarse
c  grid CV (parent). Actually, one does not need the function FICF
c  then -- it was introduced only because I did not like to split
c  an instruction over two lines ...
c******************************************************************
      DO IG=2,NIM
        IF=2*IG-2
        DO JG=2,NJM
...
c
c**************************************************************
c  In the case of an unstructured grid, one needs to create a
c  list of internal CV-faces, just as was done here with the
c  O- and C-grid "cuts": for each CV face, you need the indices
c  of the left and right neighbor CV, interpolation factor,
c  surface vector components (convention: vector points from
c  left neighbor to right neighbor), mass flux, two coefficients. (below,
c  I stored IJOC1(I) and IJOC2(I), which are the indices of grid
c  points at cell-face ends, which are used to compute surface vector
c  components; it is better to compute surface vector components
c  in the grid generator and save them -- the coding is then same
c  for 2D and 3D).
c
c  The loops over east and north internal faces and over O- and
c  C-cuts can then be replaced by a single loop which runs over
c  all inner faces; the loops over boundary faces, which need to 
c  be sorted according to boundary type, remain as below. For
c  example, for the east face, left neighbor is IJ, right is IJ+NJ
c  and so on...
c*****************************************************************
C
...
c
c*******************************************************************
c  For all volume integrals, the two loops below running in I and J
c  direction need to be replaced by a single loop over all CVs (one
c  needs to establish such a list, in parallel to the list of faces).
c  The expressions below within the loops remain valid, for both 2D
c  and 3D grids.
c*******************************************************************
C
...
c
c*************************************************************************
c  Boundary faces need a data structure similar to inner faces; instead
c  of left and right neighbor, one has boundary node and one neighbor...
c*************************************************************************
C
...
C==============================================================
c
c  Because one face always uniquely separates two CVs, there is little
c  to be changed here when unstructured grid or 3D grid is used:
c  one operates almost exclusively with surface vector components 
c  and coordinates of the two CV centers...
c*****************************************************************
c
      INCLUDE 'param.inc'
      INCLUDE 'indexc.inc'
...
c*******************************************************************
c  Instead of passing indices of CV corners, IJ1 and IJ2, one can
c  pass previously computed surface vector components, SX and SY
c  (in 3D, also SZ).
c*******************************************************************
C
      RC=0.5*(R(IJ1)+R(IJ2))
...
c
c**************************************************************
c  What was said earlier applies also here: in CALCP it is assumed
c  that boundary mass fluxes are fixed (outlet fluxes are computed
c  from extrapolated velocities and corrected to satisfy the global
c  mass conservation, see below), so one needs only to run over all
c  inner faces -- therefore, the three loops below are replaced
c  by one loop.
c****************************************************************
C
...
C
c************************************************************************
c  This loop is not needed at all; contribution to SU and AP at each CV center
c  is computed at cell faces and one can put it immediately to the
c  AP at neighbor CV centers, as is done for O- and C-cuts, see above.
c*************************************************************************
c
      DO I=2,NIM
        DO IJ=LI(I+IST)+2,LI(I+IST)+NJM
...
c
c******************************************************************
c  Again, one needs here only one loop over inner faces...
c******************************************************************
c
        DO I=2,NIM-1
          DO IJ=LI(I+IST)+2,LI(I+IST)+NJM
...
C
c***************************************************************
c  Here, a loop over volumes is needed...
c***************************************************************
c
        DO I=2,NIM
          DO IJ=LI(I+IST)+2,LI(I+IST)+NJM
...
C==============================================================
c
c..No changes needed if surface vector components are passed,
c  see FLUXUV..
c**************************************************************
c
      INCLUDE 'param.inc'
...
c
c**************************************************************
c  In the general case, one would run over all boundary faces
c  and use the available data (boundary node, neighbor node,
c  gradient at neighbor CV center etc) to extrapolate pressure
c  to the boundary.
c***************************************************************
C
      DO I=2,NIM
...
C============================================================
c
c  You will need another solver for unstructured grids: see
c  ICCG and CGSTAB in directory "solvers" at Springer's server...
c****************************************************************
      INCLUDE 'param.inc'
      INCLUDE 'indexc.inc'
      INCLUDE 'logic.inc'
...
C===================================================================
c
c  The postprocessing code provided can easily be extended to unstructured
c  grids, but the difference between 2D and 3D is large... It is perhaps
c  best to write an interface to some commercial postprocessing software --
c  simply write the vertices and cells in the format required (most software
c  will interpolate variable values to CV corners, because that is what is
c  used for plotting, but you can do it yourself if you want...)
c************************************************************************
      INCLUDE 'param.inc'
      INCLUDE 'geo.inc'
      INCLUDE 'logic.inc'
