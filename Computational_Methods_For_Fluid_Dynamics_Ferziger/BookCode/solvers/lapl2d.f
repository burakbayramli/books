...
C
C      DO I=2,NI
C        FI(LI(I)+NJ)=XC(I)*YC(NJ)
C      END DO
C      DO J=2,NJM
C        FI(LI(NI)+J)=YC(J)*XC(NI)
C      END DO
C
...
C
      DO I=2,NIM
        DO J=2,NJM
          ...
          Q(IJ)=SIN(2*PI*XC(I)/XMAX)*SIN(2*PI*YC(J)/YMAX)*VOL
        END DO
      END DO
C
...
C
      I=2
      QUO=(XC(3)-XC(1))**2-(XC(2)-XC(1))**2
      DO J=2,NJM
        IJ=LI(I)+J
        AP(IJ)=AP(IJ)+AW(IJ)*(XC(3)-XC(1))**2/QUO
        AE(IJ)=AE(IJ)-AW(IJ)*(XC(2)-XC(1))**2/QUO
        AW(IJ)=0.
      ENDDO
      J=2
      QUO=(YC(3)-YC(1))**2-(YC(2)-YC(1))**2
      DO I=2,NIM
        IJ=LI(I)+J
        AP(IJ)=AP(IJ)+AS(IJ)*(YC(3)-YC(1))**2/QUO
        AN(IJ)=AN(IJ)-AS(IJ)*(YC(2)-YC(1))**2/QUO
        AS(IJ)=0
      ENDDO
C
C.....CHOOSE THE SOLVER
...
c        DO J=2,NJM
c          IJ=LI(I)+J
c          ERR=ERR+ABS(FI(IJ)-XC(I)*YC(J))
c        END DO
c      END DO
c      ERR=ERR/ERRNOR
c      WRITE(8,*) '     Final error norm:  ERR = ',ERR
C
...
C
C############################################################
      SUBROUTINE NEUMANN(L)
C#################################################################
C     This routine computes the values of FI at boundaries where
C     the Neumann conditions are applied; these values are not 
C     used in the computation, but MG method needs them to compute
C     the corrections properly... This is not the only possible
C     way, only one of them...
C=================================================================
      PARAMETER (NX=162,NY=162,NXY=NX*NY,NXYA=NXY+NXY/2)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      COMMON /INDI/ NI,NJ,NIM,NJM,MAXIT,LI(NX)
...
C
      ...
      NJ=NJGR(L)
c
c.....Coefficients from quadratic extrapolation to Neumann boundaries
c     (valid for a uniform grid)
c
      ALF=9./8.
      BET=-1./8.
C
C.....Neumann boundary along south side: extrapolate boundary values
C
      DO I=2,NI
        IJ=(I-1)*NJ+IJGR(L)+1
        FIA(IJ)=ALF*FIA(IJ+1)+BET*FIA(IJ+2)
      END DO
C
C.....Neumann boundary along west side: extrapolate boundary values
C
      DO J=2,NJ
        IJ=IJGR(L)+J
        FIA(IJ)=ALF*FIA(IJ+NJ)+BET*FIA(IJ+2*NJ)
      END DO
C
C.....SW-corner point: average the two nearest boundary nodes
c
      IJ=IJGR(L)+1
      FIA(IJ)=0.5*(FIA(IJ+1)+FIA(IJ+NJ))
C
      RETURN
      END
C
