...
C----------------------------------------------------------------------- 
      ...
      parameter (mnm=30)
      character name*(mnm),
     & filin*(mnm+4),filto*(mnm+4),filout*(mnm+4),filgrd*(mnm+4)
*     CHARACTER*15 FILIN,FILTO,FILOUT,FILGRD,NAME*10
C
C.....DEFINE FILES
C
      WRITE(*,*) ' '
      ...
      READ(*,'(A6)') NAME
      WRITE(*,*) ' INPUT FROM KEYBOARD (1 - YES, 0 - NO)?   '
      READ(*,*) INPTYP
C
      CALL STRLEN(NAME,NAME,NSLOV)
C
      IF(INPTYP.EQ.0) THEN
C
        FILIN=NAME(1:NSLOV)//'.gin'
        OPEN (UNIT=5,FILE=FILIN)
        REWIND 5
      ELSE
        FILTO=NAME(1:NSLOV)//'.gin'
        OPEN (UNIT=1,FILE=FILTO)
        REWIND 1
      ENDIF
C
      FILOUT=NAME(1:NSLOV)//'.got'
      FILGRD=NAME(1:NSLOV)//'.grd'
C
...
C
      ...
      npt=max(npt,1)
C
      WRITE(9,99) '      PARAMETER (NGR= ',NGR,','
      WRITE(9,99) '     *           NXA= ',NIAL,','
      WRITE(9,99) '     *           NYA= ',NJAL,','
      WRITE(9,99) '     *           NMXY=',NMIJ,','
      WRITE(9,99) '     *           NXYA=',NIJA,','
      WRITE(9,99) '     *           NXYG=',NXYG,','
      WRITE(9,99) '     *           NIA= ',NINA,','
      WRITE(9,99) '     *           NOA= ',NOT,','
      WRITE(9,99) '     *           NSA= ',NST, ','
      WRITE(9,99) '     *           NWA= ',NWT,','
      WRITE(9,99) '     *           NPA= ',NPT,','
      WRITE(9,99) '     *           NOCA=',NOCT,','
      WRITE(9,98) '     *           NFI=4)'
   99 FORMAT(A22,I5,A1)
   98 FORMAT(A23)
...
C
      PRINT *,' ENTER> LSTORE, LCALG, LPRINT, LPLOT, LAXIS, LSMO:'
      READ(5,*,err=101) LSTORE,LCALG,LPRINT,LPLOT,LAXIS,LSMO
      PRINT *,' ENTER> NUMBER OF GRID LEVELS, NGR:  '
      READ(5,*,err=101) NGR
      PRINT *,' SELECTION OF STRAIGHT LINES: 0 -> S-N, 1 -> W-E:  '
      READ(5,*,err=101) IDIR 
      WRITE(1,*) LSTORE,LCALG,LPRINT,LPLOT,LAXIS,LSMO,
     *'               LSTORE,LCALG,LPRINT,LPLOT,LAXIS,LSMO'
      WRITE(1,'(i2,22x,a)') NGR,' NGR'
      WRITE(1,'(i2,22x,a)') IDIR,' IDIR'
C
      IGR(1)=0
      JGR(1)=0
      IJGR(1)=0
...
C
      RETURN                                   
  101 write(*,*) '  ** Error: Problem reading input data'
      stop
      END
C
C
...
C
      ...
      READ(5,*,err=101) NICV,NJCV
      WRITE(1,'(2i6,12x,a)') NICV,NJCV,' NICV,NJCV'
C
...
C
      DO L=1,4
        ...
        READ(5,*,err=101) NLINES(L)
        WRITE(1,'(i4,20x,a)') NLINES(L),' NLINES(L)'
        PRINT *,' COORDINATES OF BEGIN OF LINE 1:  '
        READ(5,*,err=101) XLS,YLS
        WRITE(1,'(1p2e11.3,2x,a)') XLS,YLS,' XLS,YLS'
C
C.....COORD. OF LINE END 
C
        DO LL=1,NLINES(L)
          PRINT *,' COORDINATES OF LINE ',LL,'  END:  '
          READ(5,*,err=101) XLE,YLE
          WRITE(1,'(1p2e11.3,2x,a)') XLE,YLE,' XLE,YLE'
C
C.....READ LINE SPECIFICATIONS
C
   50     PRINT *,' NUMBER OF SEGMENTS, LINE TYPE, BOUNDARY TYPE:  '
          READ(5,*,err=101) NSEG,LTYP,LBTYP
          WRITE(1,'(i5,2i3,13x,a)') NSEG,LTYP,LBTYP,' NSEG, LTYP, LBTYP'
          NCF(L)=NCF(L)+NSEG
     ...
     *       (L.GT.2.AND.NCF(L).GT.NJCV)) THEN
          ...
          READ(5,*,err=101) DX1,EXP
          WRITE(1,'(1p2e11.3,2x,a)') DX1,EXP,' DX1,EXP'
C
...
C
          IF(LTYP.EQ.2) THEN
            ...
            READ(5,*,err=101) XLM,YLM
            WRITE(1,'(1p2e11.3,2x,a)') XLM,YLM,' XLM,YLM'
            PRINT *,' ENTER ANGLES AT LINE BEGIN AND END:  '
            READ(5,*,err=101) FIS,FIE
            WRITE(1,'(1p2e11.3,2x,a)') FIS,FIE,' FIS,FIE'
C
            CALL CIRCLIN(L)
          ENDIF
C
...
C
      RETURN
  101 write(*,*) '  ** Error with reading data from input file'
      stop
      END
C
C
...
C
      DO I=1,NSEG+1
        ...
        READ(5,*,err=101) XPT(I),YPT(I)
        WRITE(1,'(1p2e11.3,2x,a)') XPT(I),YPT(I),' XPT(I),YPT(I)'
      END DO
C
C.....ASSIGN LINE POINTS TO GRID POINTS
C
...
C
        ips(k)=npt
        call defbc(6,k,npt,npre,ijpb,ijpp,ijp1,ijp2)
        npt=npt+npre(k)
C
...
C============================================================
      ...
      INCLUDE 'lines.ing'
C
      DIMENSION NB(MNG),IJB(5000),IJBP(5000),
     *          IJ1(5000),IJ2(5000)
C
...
C
        XMIN=50.
        YMIN=50.
        XMAX=XMIN+XL*SCF*0.03
        YMAX=YMIN+YL*SCF*0.03
C
...
C
        WRITE(7,*) '3 w'
        IJ=LI(1+IST)+1
        WRITE(7,*) IX(IJ),IY(IJ),' m '
C
        DO J=2,NJM
          IJ=LI(1+IST)+J
          WRITE(7,*) IX(IJ),IY(IJ),' l '
        END DO
C
        DO I=2,NIM
          IJ=LI(I+IST)+NJM
...
C==========================================================
      INCLUDE 'float.inc'
C
      WRITE(7,'(a)') '%!PS-Adobe-2.0'
      WRITE(7,'(a)') '%%Creator: GRIDPL'
      WRITE(7,'(a15,4F10.1)') '%%BoundingBox: ',
     &                        XMIN-20.,YMIN-20.,XMAX+20.,YMAX+20.
      ...
      WRITE(7,*) '50 50 translate 0.03 0.03 scale '
