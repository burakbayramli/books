...
C
      DO I=2,NIM
        DO IJ=LI(I+IST)+2,LI(I+IST)+NJM
          ...
          AP(IJ)=AP(IJ)+DEN(IJ)*VOL(IJ)*ED(IJ)/(TE(IJ)+SMALL)
        END DO
      END DO
C
C.....WALL BOUNDARIES
...
C
      DO I=2,NIM
        DO IJ=LI(I+IST)+2,LI(I+IST)+NJM
          TMP=ED(IJ)*VOL(IJ)/(TE(IJ)+SMALL)
          SU(IJ)=SU(IJ)+CE1*TMP*GEN(IJ)
          AP(IJ)=AP(IJ)+CE2*TMP*DEN(IJ)
        END DO
      END DO
C
...
C
      DO I=2,NIM
        DO IJ=LI(I+IST)+2,LI(I+IST)+NJM
          VISOLD=VIS(IJ)
          VIS(IJ)=(VISC+CMU*DEN(IJ)*TE(IJ)**2/(ED(IJ)+SMALL))*URF(IVIS)
     *            +VISOLD*(1.-URF(IVIS))
        END DO
      END DO
C
...
C   7.  FIELD INITIALIZATION (UIN,VIN,PIN,TIN,TEIN,EDIN) AND LID VELOCITY;
...
C    
      ...
      WRITE(2,*) '    '
      WRITE(2,*) '     K-EPSILON TURBULENCE MODEL WITH WALL FUNCTIONS '
      WRITE(2,*) '    ' 
