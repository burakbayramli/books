...
double omega = 1.0/ (4.0/1.8 - (4.0-1.0)*0.5);
...
{
    std::string filename = "film_uniform/" + fname + ".dat";
...
void update_bounce_back()
{
    ...
    for(int iY = 0; iY < NY; iY++)
    {
        ...
        int offset = (iY * NX + NX - 1) * NPOP;
           ...
           // Uncomment it if you want to add velocity, however it doesn't add any impact
        f2[offset + 3] = - f2[(iY * NX + NX - 2) * NPOP + 1] + 2 * weights[3] * conc_wall; //* (1.0 + 3.0 * cy[3] * u0 + 4.5 * (cy[3] * cy[3] - 1.0 / 3.0) * u0 * u0);
        f2[offset + 6] = - f2[(ytop * NX +  NX - 2) * NPOP + 8] + 2 * weights[6] * conc_wall; // * (1.0 + 3.0 * cy[6] * u0 + 4.5 * (cy[6] * cy[6] - 1.0 / 3.0) * u0 * u0);
        f2[offset + 7] = - f2[(ybottom * NX + NX - 2) * NPOP + 5] + 2 * weights[7] * conc_wall; // * (1.0 + 3.0 * cy[7] * u0 + 4.5 * (cy[7] * cy[7] - 1.0 / 3.0) * u0 * u0);
    ...
    /*for(int iX = 1; iX < NX-1; iX++)
    {
        ...
        f2[offset + 4] = f2[offset2 + 2];
        f2[offset + 7] = f2[((NY-2)*NX + iX - 1) * NPOP + 8];
        f2[offset + 8] = f2[((NY-2)*NX + iX + 1) * NPOP + 7];
    }*/
...
int main(int argc, char* argv[])
{
    ...
    for(int counter=0;counter<=N;counter++)
    {
        ...
        if (counter%NOUTPUT==0)
        {
            ...
            filewritedensity<<"film_uniform"<<std::string(7-counterconvert.str().size(),'0')<<counter;
