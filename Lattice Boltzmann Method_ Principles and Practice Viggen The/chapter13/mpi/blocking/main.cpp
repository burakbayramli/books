...
int main(int argc, char* argv[])
{
    ...
    for(unsigned int n = 0; n < NSTEPS; ++n)
    {
        ...
        // MPI_STATUS_IGNORE unavailable in MPI 1.3
#ifndef MPI_STATUS_IGNORE
        MPI_Status MPI_IGNORED_STATUS;
        MPI_Status *MPI_STATUS_IGNORE = &MPI_IGNORED_STATUS;
#endif

        // transfer data
        MPI_Sendrecv(&f1[fieldn_index(0,rank_ny-1,1)],transfer_doubles,MPI_DOUBLE,rankp1,rank,
                     &f1[fieldn_index(0,       -1,1)],transfer_doubles,MPI_DOUBLE,rankm1,rankm1,
                     MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        MPI_Sendrecv(&f1[fieldn_index(0,      0,1)],transfer_doubles,MPI_DOUBLE,rankm1,rank,
                     &f1[fieldn_index(0,rank_ny,1)],transfer_doubles,MPI_DOUBLE,rankp1,rankp1,
                     MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        
        // stream and collide from f1 storing to f2
        // optionally compute and save moments
        stream_collide_save(f0,f1,f2,rho,ux,uy,need_scalars,rank_ny);
