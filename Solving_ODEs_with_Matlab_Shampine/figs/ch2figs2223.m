function ch2ex2
% Poincare' sections example HARMOSCIL 
% of Phaser Manual.  See Lessons 13, 14.
...
%       plot(xe(event1,1),xe(event1,3),'*');
        plot(xe(event1,1),xe(event1,3),'*k','MarkerSize',2);
        %print -depsc ch2fig2b
    end
    event2 = find(ie == 2);
    if isempty(event2)
...
%       plot(xe(event2,1),xe(event2,2),'*');
        plot(xe(event2,1),xe(event2,2),'*k','MarkerSize',2);
    end
end
%==============================================================
