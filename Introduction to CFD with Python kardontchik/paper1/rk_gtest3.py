"""
   Usage: run rk_gtest3
        ...
        Solves test # 3 in Toro's chapter 6, Table 6.2 p225
        Compare also with Toro, chapter 14, Figures 14.10 to 14.18.

        Uses the ANRS_GODUNOV solver        
...
from RK_ENO_GODUNOV_SOLVER import *
...
dL, uL, pL = 1.0,     0.0,    1000.0
dR, uR, pR = 1.0,     0.0,    0.01
...
# for this test the solution extends to negative xx. Hence
# the graph is bypassed
# graphs = Graphics01(test_num,sim_time,xx,dd,uu,pp,ee)
x01 = xx; d01 = dd; u01 = uu; p01 = pp; e01 = ee
# -------------------------------------------------------
#           End of Exact RS for 1D Euler 
# -------------------------------------------------------

# -------------------------------------------------------
#           RK_ENO_GODUNOV_SOLVER_1D 
# -------------------------------------------------------
interp = 0 # solution with no ENO
wa = RK_ENO_GODUNOV_SOLVER_1D(CFL,dx,p_ic,d_ic,u_ic,v_ic,interp)
