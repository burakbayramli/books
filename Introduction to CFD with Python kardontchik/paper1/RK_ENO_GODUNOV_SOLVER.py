...
class RK_ENO_GODUNOV_SOLVER_1D:
    """
    Solves the 1D Euler equations using Eq (9.2) and the
    ANRS_GODUNOV_FLUX calculator. The Left and Right states for the
    ANRS_GODUNOV solver use the reconstructed values obtained by ENO
    """
    ...
    def calc_eno_godunov_flux(self,u1,u2,u3,u4):
        interp = self.interp

        vflux = np.zeros(4)
        f1 = np.zeros(len(u1)-1)
        f2 = np.zeros(len(f1))
        f3 = np.zeros(len(f1))
        f4 = np.zeros(len(f1))
        ...
        if interp == 1:   # HLLC with ENO
            ...
            for i in range(2,len(u1)-3): # except in the ghost cells
            ...
            La = np.sqrt(gamma*(Lp/Ld))  # not needed but gives a flag for error
            Rd = Ru1
            Ru = Ru2/Ru1
            Rv = Ru3/Ru1
            Rp = (gamma - 1.0)*(Ru4 - 0.5*(Ru2**2 + Ru3**2)/Ru1)  # Eq in p89
            Ra = np.sqrt(gamma*(Rp/Rd))  # not needed but gives a flag for error
        # solve the RP using ANRS_GODUNOV
        for i in range(2,len(u1)-3): # except in the ghost cells
            if interp == 1:
                TOL = 1.0e-6
            ...
            if pL < 0 or pR < 0:
                print 'negative pressure in RK_ENO_GODUNOV_SOLVER_1D'
                print 'pL = %g, pR = %g' % (pL,pR)
            if dL < 0 or dR < 0:
                print 'negative density in RK_ENO_GODUNOV_SOLVER_1D'
                print 'dL = %g, dR = %g' % (dL,dR)
            flu = ANRS_GODUNOV_FLUX(dL,uL,vL,pL,dR,uR,vR,pR)
            vflux,Sm = flu()
            f1[i] = vflux[0]
            f2[i] = vflux[1]
            f3[i] = vflux[2]
    ...
    def __call__(self,t):
        ...
        # Runge-Kutta
        one_u1 = np.zeros(len(u1))
        one_u2 = np.zeros(len(u1))
        one_u3 = np.zeros(len(u1))
        one_u4 = np.zeros(len(u1))
        one_f1 = np.zeros(len(f1))
        one_f2 = np.zeros(len(f1))
        one_f3 = np.zeros(len(f1))
        one_f4 = np.zeros(len(f1))

        two_u1 = np.zeros(len(u1))
        two_u2 = np.zeros(len(u1))
        two_u3 = np.zeros(len(u1))
        two_u4 = np.zeros(len(u1))
        two_f1 = np.zeros(len(f1))
        two_f2 = np.zeros(len(f1))
        two_f3 = np.zeros(len(f1))
        two_f4 = np.zeros(len(f1))
        ...
        while (tn <= t):
            ...
            one_u1[3:-3] = u1[3:-3] + (dt/dx)*(f1[2:-3] - f1[3:-2])
            one_u2[3:-3] = u2[3:-3] + (dt/dx)*(f2[2:-3] - f2[3:-2])
            one_u3[3:-3] = u3[3:-3] + (dt/dx)*(f3[2:-3] - f3[3:-2])
            one_u4[3:-3] = u4[3:-3] + (dt/dx)*(f4[2:-3] - f4[3:-2])                   
            # update ghost cells: transmissive boundary conditions
            one_u1[0] = one_u1[1] = one_u1[2] = one_u1[3]
            one_u2[0] = one_u2[1] = one_u2[2] = one_u2[3]
            one_u3[0] = one_u3[1] = one_u3[2] = one_u3[3]
            one_u4[0] = one_u4[1] = one_u4[2] = one_u4[3]
            one_u1[-1] = one_u1[-2] = one_u1[-3] = one_u1[-4]
            one_u2[-1] = one_u2[-2] = one_u2[-3] = one_u2[-4]
            one_u3[-1] = one_u3[-2] = one_u3[-3] = one_u3[-4]
            one_u4[-1] = one_u4[-2] = one_u4[-3] = one_u4[-4]

            # Step II:
            # given the new u[i]'s, reconstruct the new u[i+1/2]'s and calculate the new fluxes
            one_f1,one_f2,one_f3,one_f4 = self.calc_eno_godunov_flux(one_u1,one_u2,one_u3,one_u4)
            
            two_u1[3:-3] = 0.75*u1[3:-3] + 0.25*one_u1[3:-3] + 0.25*(dt/dx)*(one_f1[2:-3] - one_f1[3:-2])
            two_u2[3:-3] = 0.75*u2[3:-3] + 0.25*one_u2[3:-3] + 0.25*(dt/dx)*(one_f2[2:-3] - one_f2[3:-2])
            two_u3[3:-3] = 0.75*u3[3:-3] + 0.25*one_u3[3:-3] + 0.25*(dt/dx)*(one_f3[2:-3] - one_f3[3:-2])
            two_u4[3:-3] = 0.75*u4[3:-3] + 0.25*one_u4[3:-3] + 0.25*(dt/dx)*(one_f4[2:-3] - one_f4[3:-2])      
            # update ghost cells: transmissive boundary conditions
            two_u1[0] = two_u1[1] = two_u1[2] = two_u1[3]
            two_u2[0] = two_u2[1] = two_u2[2] = two_u2[3]
            two_u3[0] = two_u3[1] = two_u3[2] = two_u3[3]
            two_u4[0] = two_u4[1] = two_u4[2] = two_u4[3]
            two_u1[-1] = two_u1[-2] = two_u1[-3] = two_u1[-4]
            two_u2[-1] = two_u2[-2] = two_u2[-3] = two_u2[-4]
            two_u3[-1] = two_u3[-2] = two_u3[-3] = two_u3[-4]
            two_u4[-1] = two_u4[-2] = two_u4[-3] = two_u4[-4]

            # Step III:
            # given the new u[i]'s, reconstruct the new u[i+1/2]'s and calculate the new fluxes
            two_f1,two_f2,two_f3,two_f4 = self.calc_eno_godunov_flux(two_u1,two_u2,two_u3,two_u4)
            
            c13 = 1.0/3.0; c23 = 2.0/3.0
            u1[3:-3] = c13*u1[3:-3] + c23*two_u1[3:-3] + c23*(dt/dx)*(two_f1[2:-3] - two_f1[3:-2])
            u2[3:-3] = c13*u2[3:-3] + c23*two_u2[3:-3] + c23*(dt/dx)*(two_f2[2:-3] - two_f2[3:-2])
            u3[3:-3] = c13*u3[3:-3] + c23*two_u3[3:-3] + c23*(dt/dx)*(two_f3[2:-3] - two_f3[3:-2])
            u4[3:-3] = c13*u4[3:-3] + c23*two_u4[3:-3] + c23*(dt/dx)*(two_f4[2:-3] - two_f4[3:-2]) 
            # update ghost cells: transmissive boundary conditions
            u1[0] = u1[1] = u1[2] = u1[3]
            u2[0] = u2[1] = u2[2] = u2[3]
            u3[0] = u3[1] = u3[2] = u3[3]
            ...
            if interp == 0:
                ...
                v = u3/u1
                p = (gamma - 1.0)*(u4 - 0.5*(u2**2 + u3**2)/u1)  # Eq in p89
                a = np.sqrt(gamma*(p/d))  # not needed but gives a flag for warning
