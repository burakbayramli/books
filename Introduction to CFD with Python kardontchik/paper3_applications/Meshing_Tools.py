...
def boundary_angles(p,boundary_nodes):  
    ...
    ind = np.where(boundary_nodes == -1)
    if len(ind[0]):  # found the -1; we have two boundaries
        # close the boundary polygon for the external boundary
        index = int(ind[0])
        # close the external boundary polygon
        # len(ext_boundary_nodes) = index
        ext_boundary_nodes = boundary_nodes[0:index]
        
        cext_boundary_nodes = np.zeros(len(ext_boundary_nodes)+1,dtype=int)
        cext_boundary_nodes[0:-1] = ext_boundary_nodes
        cext_boundary_nodes[-1] = ext_boundary_nodes[0]

        xx0 =p[ext_boundary_nodes,0]; yy0 = p[ext_boundary_nodes,1]
        xx1 = p[cext_boundary_nodes[1:],0]; yy1 = p[cext_boundary_nodes[1:],1]
        # angle of the normal (pointing towards the ghost triangle), in radians
        ext_boundary_angles = np.arctan2(xx0 - xx1,yy1 - yy0)
        
        # close the internal boundary polygon
        int_boundary_nodes = boundary_nodes[index+1:]

        cint_boundary_nodes = np.zeros(len(int_boundary_nodes)+1,dtype=int)
        cint_boundary_nodes[0:-1] = int_boundary_nodes
        cint_boundary_nodes[-1] = int_boundary_nodes[0]

        xx0 =p[int_boundary_nodes,0]; yy0 = p[int_boundary_nodes,1]
        xx1 = p[cint_boundary_nodes[1:],0]; yy1 = p[cint_boundary_nodes[1:],1]
        # angle of the normal (pointing towards the ghost triangle), in radians
        int_boundary_angles = np.arctan2(xx0 - xx1,yy1 - yy0)

        boundary_angles = np.concatenate((ext_boundary_angles,int_boundary_angles))
