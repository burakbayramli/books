% IF_unp.m
% Usage: Mach = IF_unp(Pratio <, gamma>) where gamma=1.4 if omitted
% Function for Mach number from isentropic stagnation to static pressure ratio.
% S. Collicott, Oct. 2010
% Copyright (C) 2010, Purdue University
%
function [Mach] = IF_unp(arg1, varargin)
gam = 1.4;  % default ratio of specific heats
Mach = NaN;
prat = arg1; % Mach number
if isempty(find(prat < 1, 1)) && length(varargin) < 2
  if length(varargin)==1
    tmp = cell2mat(varargin);
    if tmp > 0
      gam = tmp;
    else
      gam = NaN;
    end
  end
  %
  Mach = sqrt( (prat.^((gam-1)/gam)-1)*(2/(gam-1)) );
end
