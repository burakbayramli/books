...
k = 0.5; %thermal condcutivity of material units must be in W/m-K
A = 1; %perpendicular area units must be m^2
L = 0.02; %length of 1D problem
q = 1000*10^3; %heat generation must be W/m^3
n = 5; %node points
dx = L/n; %distance between points
Ta = 100; %temperature at A
Tb = 200; %temperature at B
...
for i = 2:n-1 %for loop for making array for ap & su coeffcients
    ap(i,1) = (2*k*A)/dx; %ap coefficient value from 2 to n-1 nodes
    su(i,1) = q*A*dx; %su coefficient value from 2 to n-1 nodes
end
for i = 1:n %for loop for making array for ap su & sp coeffcients
    if i==1 %if condition for node 1
        su(i,1) = q*A*dx + ((2*k*A)/dx)*Ta; %su value at node 1
        sp(i,1) = -((2*k*A)/dx); %sp value at node 1
        ap(i,1) = (3*k*A)/dx; %ap value at node 1
    elseif i==n %elseif condition for node 1
        su(i,1) = q*A*dx + ((2*k*A)/dx)*Tb; %su value at node n
...
for i = 2:n-1 %for loop for making arrays S & B
    ...
    B(i,1) = q*A*dx; %su value at nodes 2 to n-1
    d(i,1) = q*A*dx; %su value at nodes 2 to n-1
end
...
figure (1)
plot(x,T,'*')
title('Temeperature Plot for 1D steady state heat conduction with heatgeneration')
xlabel('X [m]')
