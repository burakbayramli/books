      SUBROUTINE Conver( iter,imax,ib2,ncells,a,cv,cvold,p )
...
C --- get the change of density and the mass flow
      ...
      ENDDO
      avms = avms/REAL(ncells+1)

C --- print convergence history

      IF (iter .EQ. 1) drho1 = SQRT(drho/REAL(ncells+1))
      drho = SQRT(drho/REAL(ncells+1))/drho1
