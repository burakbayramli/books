//OpenCFD EC 2D in c 
// write by cofludy refer to Li XINLIANG's code V1.5.4
//Li XiLiang LHD, Institute of Mechanics, lixl@imech.ac.cn
//Cofludy  cofludy@gmail.com
...
int main()
{
	printf("----------------- OpenCFD-EC2D in C ver 2.0.0 --------------------------\n");
	printf("               Copyright by Li Xinliang, lixl@imech.ac.cn\n");
	...
	for (; Mesh[1].tt < t_end; ) {
		if (Num_Mesh== 1)  {                          //单重网格时间推进(1阶Euler, 3阶RK, LU - SGS)
			NS_Time_advance(1);
		}
		else  if (Num_Mesh== 2)  {                    //时间推进，采用2重网格法
			//NS_2stge_multigrid();
		}
		else{										    //时间推进，采用3重网格法
			//NS_3stge_multigrid();
