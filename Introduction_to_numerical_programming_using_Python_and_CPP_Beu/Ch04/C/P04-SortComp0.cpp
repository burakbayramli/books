...
int main()
{
   ...
   out = fopen("sort.txt","w");                           // open output file
   fprintf(out,"       n            Bubble              "
               "Insertion           Quick\n");
   fprintf(out,"               ncomp     nsave     ncomp"
               "     nsave     ncomp     nsave\n");
   for (ip=1; ip<=np; ip++) {
      ...
      ncomp = 0; nsave = 0;
      BubbleSort(x,n);
      fprintf(out,"%10i%10i%10i",n,ncomp,nsave);

      for (i=1; i<=n; i++) x[i] = x0[i];
      ncomp = 0; nsave = 0;
      InsertSort(x,n);
      fprintf(out,"%10i%10i",ncomp,nsave);
