...
{
   double *V, *x, *y;
   double d, dE, dy0, E, Emin, Emax, Ew, eps, f, hx, xc, xx, y0, yH;
   int exist, i, m, n, nc, nx, par;
   FILE *out;
   ...
   if (exist) {
      ...
      for (m=1; m<=nc; m++) f += y[m]*y[m]*hx;            // norm on [0,+inf]
      f = 1e0/sqrt(2e0*f);
      if (int(n/2) % 2) f = -f;                            // sign correction
      for (m=1; m<=nc; m++) y[m] *= f;

      f = sqrt(pi);                            // norm of Hermite polynomials
      for (i=1; i<=n; i++) f *= 2e0 * i;
      f = 1e0/sqrt(f);

      out = fopen("shoot.txt","w");
      fprintf(out,"E%i = %8.5f\n",n,E);
      fprintf(out,"      x        y%i        err\n",n);
      for (m=1; m<=nc; m++) {
         yH = f * exp(-0.5*x[m]*x[m]) * Hermite(n,x[m],d);
