...
f_c = moving_wall_bc_all(f_c,u_lb_c,'north');
%     f_c = wall_bc(f_c,'north');
f_c = wall_bc(f_c,'west');
f_c = wall_bc(f_c,'south');
f_f = wall_bc(f_f,'east');
%     f_f = wall_bc(f_f,'north');
f_f = moving_wall_bc_all(f_f,u_lb_f,'north');
...
%     f_f = wall_bc(f_f,'north');
    ...
    % Stream coarse and fine+interface
    f_c = stream(f_c);
    % Stream fine cell data to the interface.
    f_if = stream([f_i, f_f]);
    f_i = f_if(:,1:2,:);
    f_f = f_if(:,3:end,:);
