...
%
 format long g
%
% This is where the to input the data in the form of a file with
% an extension .m 
%
Q8_coarse_mesh_data
%
...
%
for i=1:nel
    [coord,g] = elem_q8(i);         % coordinates of the nodes of element i, and its steering vector 
    eld=zeros(eldof,1);             % Initialise element displacement to zero
    for m=1:eldof                   %
        if g(m)==0                  %
            eld(m)=0.;              %
        else                        %
            eld(m)=delta(g(m));     % Retrieve element displacement from the global displacement vector
        end
    end
%
    for ig=1: ngp
    ...
    for jg=1: ngp
        wj=samp(jg,2);
        [der,fun] = fmquad(samp, ig,jg);    % Derivative of shape functions in local coordinates
        jac=der*coord;                      % Compute Jacobian matrix
        jac1=inv(jac);                      % Compute inverse of the Jacobian
