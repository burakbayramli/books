...
% SIM.LargeScale: if 0, set LargeScale to off; else use LargeScale on
%
...
%               optimal design
%
% F: the optimal value of the cost function
%
% Kenneth J. Beers
% MIT Department of Chemical Engineering
% May 31, 2005.  Version June 9, 2005. B

function [TRAJ,F,iflag] = optimal_control(FUN,PARAM,SIM,TRAJ0);
iflag = 0;
...
% set LargeScale to default if necessary
if(~isfield(SIM,'LargeScale'))
    SIM.LargeScale = 1;  % use LargeScale as default
end
...
% set options structure for minimizer
if(SIM.verbose == 0)
...
if(SIM.LargeScale == 0)
    OPTIMOPT = optimset(OPTIMOPT,'LargeScale','off');
else
    OPTIMOPT = optimset(OPTIMOPT,'LargeScale','on');
end
