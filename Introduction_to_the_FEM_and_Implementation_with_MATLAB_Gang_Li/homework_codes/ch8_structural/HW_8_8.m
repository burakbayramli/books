...
% next block: generate nodes
nodes=zeros(3*n_division+1,4);
nodes(1:n_division+1,2:3)=[zeros(1,n_division+1);0:10/n_division:10]';
nodes(n_division+1:2*n_division+1,2:3)=[0:8/n_division:8; ones(1,n_division+1)*10]';
nodes(2*n_division+1:3*n_division+1,2:3)=[ones(1,n_division+1)*8;...
                                          10:-3/n_division:7]';
...
% next block: materials=[E G]
materials=[2e9 8e8]'*0.000145038;    %conver to psi

% next block: cross-sectional properties
% geometry row elements: element id, alpha, A, Iy, Iz, J
geometry=zeros(n_elements,6);
geometry(:,1)=[1:n_elements]';
geometry(:,3)=1;
geometry(:,4)=8.3333e-02;
geometry(:,5)=8.3333e-02;
geometry(:,6)=1.6667e-01;

forces(1,:)=[3*n_division+1 0 -100 0 0 0 0];

% next block: set up displacement boundary conditions
bcsdisp=zeros(6,3);
