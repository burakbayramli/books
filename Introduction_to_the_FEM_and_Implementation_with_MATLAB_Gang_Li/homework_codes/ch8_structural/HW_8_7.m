...
materials=[70e9 28e9]';    % G value can be arbitrary
...
forces=zeros(n_nodes,7);
forces(1:n_nodes,1)=[1:n_nodes]';
forces(1:n_nodes,3)=-w/n_division;
forces(1,3)=-w/n_division/2;
forces(n_nodes,3)=-w/n_division/2;
bcsdisp=zeros(6,3);
...
for e=1:nElements
  ...
  plot3([x1+d(6*nd1-5) x2+d(6*nd2-5)],[y1+d(6*nd1-3) ...
         ...
         'k-','LineWidth',4);
  plot3([x1+d(6*nd1-5) x2+d(6*nd2-5)],[0,0],[-7.1429e-04*w*x1^2*(x1^2-4*x1+6) ...
         -7.1429e-04*w*x2^2*(x2^2-4*x2+6)],...
         'r-','LineWidth',2);
end
