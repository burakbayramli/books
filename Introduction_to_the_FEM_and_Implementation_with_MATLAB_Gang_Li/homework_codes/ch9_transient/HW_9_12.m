...
n_div=1;  % number of divisions for each of the straight segment

% next block: geometric model
keypts=[1 0  20 0; 2 20 20 0; 3 20 20 20; 4 20 0 20];
segments=[1 1 2; 2 2 3; 3 3 4];
...
materials=[2e7 8e7 1]';  % material properties
...
geometry(:,5)=0.25;
geometry(:,6)=2.5;
geometry(n_elements*2/3+1:n_elements,2)=-90;

% next block: displacement boundary conditions
bcsdisp=zeros(12,3);
bcsdisp(1:6,1:2)=[ones(1,6);[1 2 3 4 5 6]]';
bcsdisp(7:12,1:2)=[ones(1,6)*4;[1 2 3 4 5 6]]';
...
for e=1:nElements
  ...
  k=k+k'-eye(12).*diag(k);
  % next 10 lines: element m matrix in local coordinate system 
  me=zeros(12,12);
