...
conv3 = tf.layers.conv1d(inputs=max_pool_2, filters=72, kernel_size=3, strides=2, 
                         padding='same', activation = tf.nn.relu)
max_pool_3 = tf.layers.max_pooling1d(inputs=conv3, pool_size=2, strides=2, padding='same')

# (batch, 16, 72) --> (batch, 8, 144)
conv4 = tf.layers.conv1d(inputs=max_pool_3, filters=144, kernel_size=3, strides=2,
                         padding='same', activation = tf.nn.relu)

max_pool_4 = tf.layers.max_pooling1d(inputs=conv4, pool_size=2, strides=2, padding='same')
print max_pool_4.shape
flat = tf.reshape(max_pool_4, (-1, 250*144))

logits = tf.contrib.layers.fully_connected(inputs=flat,
                                           num_outputs=12,
                                           activation_fn=None)
