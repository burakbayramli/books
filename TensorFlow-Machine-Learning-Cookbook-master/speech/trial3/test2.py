...
conv1 = tf.layers.conv1d(inputs=pcm, filters=filters,
...
conv2 = tf.layers.conv1d(inputs=max_pool_1, filters=filters,
                         kernel_size=kernel_size,
                         strides=1, 
                         padding='same',
                         activation = tf.nn.relu)
...
output, states = tf.nn.dynamic_rnn(cell, max_pool_2, dtype=tf.float32)
last = states[-1][0]
