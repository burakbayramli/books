...
conv1 = tf.layers.conv1d(inputs=pcm,filters=10,kernel_size=4,strides=2, padding='same',activation = tf.nn.relu)

max_pool_1 = tf.layers.max_pooling1d(inputs=conv1, pool_size=2, strides=2, padding='same')

conv2 = tf.layers.conv1d(inputs=max_pool_1,filters=10,kernel_size=2,strides=2, padding='same',activation = tf.nn.relu)

max_pool_2 = tf.layers.max_pooling1d(inputs=conv2, pool_size=2, strides=2, padding='same')

conv3 = tf.layers.conv1d(inputs=max_pool_2,filters=10,kernel_size=2,strides=2, padding='same',activation = tf.nn.relu)
...
drop = tf.nn.dropout(max_pool_3, keep_prob=0.8)
...
output, states = tf.nn.dynamic_rnn(cell, drop, dtype=tf.float32)
last = states[-1][0]
