...
% This program applies Gradient Projection with nonmonotone version of 
% Barzilai-Borwein steplength rule to solve the dual formulation of 
...
function [u, w1,w2, Energy, Dgap, TimeCost, itr] = ...
      TV_GPBBNM(w1,w2,f,lbd,NIT,GapTol,verbose);
...
%  Dgap(itr+1)= sum(sum(gu_norm + ux.*w1 + uy.*w2)); 
  ...
  if verbose
    fprintf(' GPBBNM itr %4d: Pri=%8.3e, Dua=%8.3e, Gap=%5.2e\n', itr, ...
  ...
  if (Dgap(itr+1) < GapTol )
    if verbose
      fprintf(1,' GPBBNM convergence tolerance %7.3e satisfied\n', GapTol);
    end
    break
  end   
end
