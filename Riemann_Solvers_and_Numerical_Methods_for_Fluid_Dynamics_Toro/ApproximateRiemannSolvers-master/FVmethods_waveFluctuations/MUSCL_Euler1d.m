...
while t<tFinal
    ...
    % 1st stage
    dF=MUSCL_EulerRes1d_Fluctuations(q,dx,lim);	q = qo-dt*dF; 
    q(:,1)=qo(:,2); q(:,end)=qo(:,end-1);	% Neumann BCs
    
    % 2nd Stage
    dF=MUSCL_EulerRes1d_Fluctuations(q,dx,lim);	q = 0.75*qo+0.25*(q-dt*dF);
    q(:,1)=qo(:,2); q(:,end)=qo(:,end-1);	% Neumann BCs

    % 3rd stage
    dF=MUSCL_EulerRes1d_Fluctuations(q,dx,lim);	q = (qo+2*(q-dt*dF))/3;
    q(:,1)=qo(:,2); q(:,end)=qo(:,end-1);	% Neumann BCs
...
title(s1,'MUSLC-FV-HLLC wave fluctuation Euler solver');