...
class InterCell
{
  ...
  public:
    ...
    void set(PrimitiveVar *left, PrimitiveVar *right)
    {
        ...
        //Estimate the wave speed directly with Roe-Average
        ...
        const double S_L = u_tilde - a_tilde;
        const double S_R = u_tilde + a_tilde;
        const double S_s = (Wr->p - Wl->p + Wl->rho * Wl->u * (S_L - Wl->u) - Wr->rho * Wr->u * (S_R - Wr->u))/(Wl->rho * (S_L - Wl->u) - Wr->rho * (S_R - Wr->u));
        ...
        else
        {
            ...
            {
                local_flux.set(*Wl);
                ConservativeVar U_L(*Wl);
                const double coef = Wl->rho * (S_L - Wl->u) / (S_L - S_s);
                ConservativeVar U_sL(coef, coef*S_s, coef);
                U_sL.at(2) *= (E(*Wl)/Wl->rho + (S_s - Wl->u) * (S_s + Wl->p / Wl->rho / (S_L - Wl->u)));
                ConservativeVar dU = U_sL - U_L;
                local_flux.mass_flux() += S_L * dU.at(0);
                local_flux.momentum_flux() += S_L * dU.at(1);
                local_flux.energy_flux() += S_L * dU.at(2);
            }
            else
            {
                local_flux.set(*Wr);
                ConservativeVar U_R(*Wr);
                const double coef = Wr->rho * (S_R - Wr->u) / (S_R - S_s);
                ConservativeVar U_sR(coef, coef*S_s, coef);
                U_sR.at(2) *= (E(*Wr)/Wr->rho + (S_s - Wr->u) * (S_s + Wr->p / Wr->rho / (S_R - Wr->u)));
                ConservativeVar dU = U_sR - U_R;
                local_flux.mass_flux() += S_R * dU.at(0);
                local_flux.momentum_flux() += S_R * dU.at(1);
                local_flux.energy_flux() += S_R * dU.at(2);
            }
        }
    }
};
