...
class InterCell
{
  ...
  public:
    ...
    void set(PrimitiveVar *left, PrimitiveVar *right)
    {
        ...
        //Estimate the wave speed from approximated pressue at star region
        const double rho_bar = 0.5 * (Wl->rho + Wr->rho);
        const double a_bar = 0.5 * (Wl->a + Wr->a);
        const double p_pvrs = 0.5 * (Wl->p + Wr->p) - 0.5 * (Wr->u - Wl->u) * rho_bar * a_bar;
        const double p_s = max(0.0, p_pvrs);
        const double q_L = p_s > Wl->p ? sqrt(1+G2*(p_s/Wl->p-1)) : 1.0;
        const double q_R = p_s > Wr->p ? sqrt(1+G2*(p_s/Wr->p-1)) : 1.0;
        const double S_L = Wl->u - Wl->a * q_L;
        const double S_R = Wr->u + Wr->a * q_R;
