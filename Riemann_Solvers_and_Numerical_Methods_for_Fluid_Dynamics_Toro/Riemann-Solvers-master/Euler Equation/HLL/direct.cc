...
class InterCell
{
  ...
  public:
    ...
    void set(PrimitiveVar *left, PrimitiveVar *right)
    {
        ...
        {
            const double dS_LR = S_R - S_L;
            FluxVar F_L, F_R;
            F_L.set(*Wl);
            F_R.set(*Wr);
            ConservativeVar U_L(*Wl), U_R(*Wr);
            ConservativeVar dU = U_R - U_L;
            local_flux.mass_flux() = (S_R * F_L.mass_flux() - S_L * F_R.mass_flux() + S_L * S_R * dU.at(0))/dS_LR;
            local_flux.momentum_flux() = (S_R * F_L.momentum_flux() - S_L * F_R.momentum_flux() + S_L * S_R * dU.at(1))/dS_LR;
            local_flux.energy_flux() = (S_R * F_L.energy_flux() - S_L * F_R.energy_flux() + S_L * S_R * dU.at(2))/dS_LR;
        }
    }
};
