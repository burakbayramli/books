...
class InterCell
{
  ...
  public:
    ...
    void set(PrimitiveVar *left, PrimitiveVar *right)
    {
        ...
        //Estimate the wave speed from 1 max speed directly
        const double S_plus = max(abs(Wl->u) + Wl->a, abs(Wr->u) + Wr->a);
        const double S_L = -S_plus;
        const double S_R = S_plus;

        //Rusanov flux
        FluxVar F_L, F_R, F_LR;
        F_L.set(*Wl);
        F_R.set(*Wr);
        local_flux = F_L + F_R;
        ...
        local_flux.mass_flux() -= 0.5 * S_plus * dU.at(0);
        local_flux.momentum_flux() -= 0.5 * S_plus * dU.at(1);
        local_flux.energy_flux() -= 0.5 * S_plus * dU.at(2);
    }
};
