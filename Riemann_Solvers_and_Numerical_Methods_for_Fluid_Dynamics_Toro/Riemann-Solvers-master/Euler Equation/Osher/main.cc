...
class InterCell
{
  ...
  public:
	...
	void set(PrimitiveVar *left, PrimitiveVar *right)
	{
		...
		const double p0 = Wl->p, p1 = Wr->p;
		const double H = pow(p0 / p1, G1);
		const double u0 = Wl->u, u1 = Wr->u;
		const double a0 = Wl->a, a1 = Wr->a;
		const double ps = pow((a0 + a1 - (u1 - u0) * G7) / (a0 / pow(p0, G1) + a1 / pow(p1, G1)), G3);
		const double us = (H * u0 / a0 + u1 / a1 + 2 * (H - 1) / G8) / (H / a0 + 1 / a1);
		const double a13 = a0 * pow(ps / p0, G1);
		const double a23 = a1 * pow(ps / p1, G1);
		const double rho0 = Wl->rho, rho1 = Wr->rho;
		const double rho13 = rho0 * pow(ps / p0, G12), rho23 = rho1 * pow(ps / p1, G12);
		const double uS0 = G6 * u0 + G5 * a0;
		const double aS0 = uS0;
		const double rhoS0 = rho0 * pow(aS0 / a0, G4);
		const double pS0 = p0 * pow(rhoS0 / rho0, G0);
		const double uS1 = G6 * u1 - G5 * a1;
		const double aS1 = -uS1;
		const double rhoS1 = rho1 * pow(aS1 / a1, G4);
		const double pS1 = p1 * pow(rhoS1 / rho1, G0);

		FluxVar F0, F1, F13, F23, FS0, FS1;
		F0.set(*Wl);
		F1.set(*Wr);
		F13.set(rho13, us, ps);
		F23.set(rho23, us, ps);
		FS0.set(rhoS0, uS0, pS0);
		FS1.set(rhoS1, uS1, pS1);

		const bool cond0 = us >= 0;
		const bool cond1 = us - a13 >= 0;
		const bool cond2 = us + a23 >= 0;
		const bool cond3 = u0 - a0 >= 0;
		const bool cond4 = u1 + a1 >= 0;
		...
		switch (opt)
		{
			case 0:
				...
				break;
			case 1:
				local_flux = F1 + F13 - FS1;
				break;
			case 2:
				local_flux = F23 - FS1 + F1;
				break;
			case 3:
				local_flux = FS0 - FS1 + F1;
				break;
			case 4:
				local_flux = F0 - FS0 + F1;
				break;
			case 5:
				local_flux = F0 - FS0 + F13 - FS1 + F1;
				break;
			case 6:
				local_flux = F0 - FS0 + F23 - FS1 + F1;
				break;
			case 7:
				local_flux = F0 + F1 - FS1;
				break;
			case 8:
				local_flux = FS1;
				break;
			...
			case 11:
				...
				break;
			case 12:
				local_flux = F0 - FS0 + FS1;
				break;
			case 13:
				local_flux = F0 - FS0 + F13;
				break;
			case 14:
				local_flux = F0 - FS0 + F23;
				break;
			case 15:
				local_flux = F0;
				break;
