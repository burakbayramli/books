...
int main(int argc, char **argv)
{
    ...
    for (int c = 0; c < NumOfCase; ++c)
    {
        ...
        for (int i = 1; i < NumOfStep; i++)
        {
            ...
            //Splitting of Mach Number and Pressure
            //There'r 2 choices for pressure, we take the first one
            vector<vector<double> >Ma(2, vector<double>(NumOfPnt + 2));
            vector<vector<double> >P(2, vector<double>(NumOfPnt + 2));
            for(int j = 0; j < NumOfPnt + 2; ++j)
            {
                const double local_Ma = w[PREV][j].Ma;
                const double abs_local_Ma = fabs(local_Ma);
                const double local_p = w[PREV][j].p;
                if(abs_local_Ma <= 1.0)
                {
                    Ma[0][j] = -0.25 * pow(local_Ma - 1, 2);
                    Ma[1][j] = 0.25 * pow(local_Ma + 1, 2);
                    P[0][j] = 0.5 * local_p * (1 - local_Ma);
                    P[1][j] = 0.5 * local_p * (1 + local_Ma);
                }
                else
                {
                    Ma[0][j] = 0.5 * (local_Ma - abs_local_Ma);
                    Ma[1][j] = 0.5 * (local_Ma + abs_local_Ma);
                    P[0][j] = 0.5 * local_p * (local_Ma - abs_local_Ma) / local_Ma;
                    P[1][j] = 0.5 * local_p * (local_Ma + abs_local_Ma) / local_Ma;
                }
            }

            //Intercell Mach Number and Pressure
            vector<double> mMa(NumOfPnt + 1), mP(NumOfPnt + 1);
            for (int j = 0; j <= NumOfPnt; ++j)
            {
                mMa[j] = Ma[1][j] + Ma[0][j + 1];
                mP[j] = P[1][j] + P[0][j + 1];
            }
            ...
            for (int j = 0; j <= NumOfPnt; ++j)
            {
                //Splitting of Advection and Pressure
                mf[j].momentum_flux() = mP[j];

                //Advection Upstream
                if(mMa[j] >= 0)
                {
                    const double tmp = w[PREV][j].rho * w[PREV][j].a;
                    mf[j].mass_flux() += mMa[j] * tmp;
                    mf[j].momentum_flux() += mMa[j] * tmp * w[PREV][j].u;
                    mf[j].energy_flux() += mMa[j] * tmp * H(w[PREV][j]);
                }
                else
                {
                    const double tmp = w[PREV][j+1].rho * w[PREV][j+1].a;
                    mf[j].mass_flux() += mMa[j] * tmp;
                    mf[j].momentum_flux() += mMa[j] * tmp * w[PREV][j+1].u;
                    mf[j].energy_flux() += mMa[j] * tmp * H(w[PREV][j+1]);
                }
            }
