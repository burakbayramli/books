...
class InterCell
{
  ...
  public:
    ...
    void set(PrimitiveVar *left, PrimitiveVar *right, double S)
    {
        Wl = left;
        Wr = right;

        //2-step Lax-Wendroff intercell flux
        ConservativeVar Ul(*Wl), Ur(*Wr);
        ConservativeVar U_tmp = Ul + Ur;
        U_tmp.elem(0) *= 0.5;
        U_tmp.elem(1) *= 0.5;
        U_tmp.elem(2) *= 0.5;
        ...
        //The 1st step
        U_tmp.elem(0) += 0.5 / S * f_tmp.mass_flux();
        U_tmp.elem(1) += 0.5 / S * f_tmp.momentum_flux();
        U_tmp.elem(2) += 0.5 / S * f_tmp.energy_flux();

        //The 2nd step
        PrimitiveVar W_tmp = U_tmp.toPrimitive();
        local_flux.set(W_tmp);
    }
};
...
int main(int argc, char **argv)
{
    ...
    for (int c = 0; c < NumOfCase; ++c)
    {
        ...
        for (int i = 1; i < NumOfStep; i++)
        {
            ...
            const double cur_speed = dx / dt;
            //Calculate intercell flux
            for (int j = 0; j <= NumOfPnt; ++j)
                f[j].set(&w[PREV][j], &w[PREV][j + 1], cur_speed);
