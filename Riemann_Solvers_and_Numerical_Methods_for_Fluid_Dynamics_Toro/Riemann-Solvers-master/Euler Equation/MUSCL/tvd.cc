...
double xi_L(double r)
{
	return 2.0 * r / (1 - omega + (1 + omega) * r);
}

double xi_R(double r)
{
	return 2.0 / (1 - omega + (1 + omega) * r);
}

double phi_sb(double r)
{
	if(r <= 0)
		return 0;
	else if(r <= 0.5)
	...
	{
		double ret = min(r, 2.0);
		return min(ret, xi_R(r));
	}	
}

double phi_vl(double r)
{
	if(r <= 0)
		return 0;
	else
		return min(2*r/(1+r), xi_R(r));
}

double phi_va(double r)
{
	if(r <= 0)
		return 0;
	else
		return min((1+r)*r/(1+pow(r, 2)), xi_R(r));
}

double phi_mb(double r)
{
	if(r <= 0)
		return 0;
	else if(r <= 1)
	...
	for (int c = 0; c < NumOfCase; ++c)
	{
		...
		for (int i = 1; i < NumOfStep; i++)
		{
			...
			for(int j = 1; j <= NumOfPnt; ++j)
			{
				...
				double r = 0.0;
				double dul = w[PREV][j].u - w[PREV][j-1].u;
				double dur = w[PREV][j+1].u - w[PREV][j].u;
				if(fabs(dur) < 1e-8)
					r = 1.0 * (dul > 0 ? INT_MAX : -INT_MAX);
				else
					r = dul / dur;
				// double slope_limiter = phi_sb(r);
				// double slope_limiter = phi_mb(r);
				double slope_limiter = phi_vl(r);
				// double slope_limiter = phi_va(r);
				slope_vec[j] *= slope_limiter;
			}
