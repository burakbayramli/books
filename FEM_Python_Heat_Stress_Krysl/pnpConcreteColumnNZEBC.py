...
# Compute heat load from element 2 due to prescribed temperatures
LTe2 = zeros(3).reshape(3, 1)
for co  in arange(len(zedof)):
    if zedof[co] >= N_f:
        LTe2 = LTe2 - Ke2[:, co].reshape(3, 1)*Tfix # note the - sign
print(LTe2)
# Assemble heat load from element 2
for ro  in arange(len(zedof)):
    if zedof[ro] < N_f:
        L[zedof[ro]] = L[zedof[ro]] + LTe2[ro]
...
# Compute heat load from element 3 due to prescribed temperatures
LTe3 = zeros(3).reshape(3, 1)
for co  in arange(len(zedof)):
    if zedof[co] >= N_f:
        LTe3 = LTe3 - Ke3[:, co].reshape(3, 1)*Tfix # note the - sign
print(LTe3)
# Assemble heat load from element 1
for ro  in arange(len(zedof)):
    if zedof[ro] < N_f:
        L[zedof[ro]] = L[zedof[ro]] + LTe3[ro]
