"""
   Usage: run quad_mesh
   (eliminates the two points closest to the tail)
   Uses one-sided body_fitted mesh
   quadrilateral mesh
"""
# Save generated mesh in file:
filename = 'quad_mesh_40deg'
# include any other useful info, for ex: 'quad_mesh_40deg'
...
for q in quad:
    ...
    plt.hold('on')
plt.title('BODY-FITTED QUAD MESH, ONE-SIDED, BETA = %g' % (beta))
plt.hold('off')
plt.show()    
