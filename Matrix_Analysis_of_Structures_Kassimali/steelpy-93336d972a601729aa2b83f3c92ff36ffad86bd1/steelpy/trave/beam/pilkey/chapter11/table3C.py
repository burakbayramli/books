...
class ArbitrarySupport(TableBasic):
    ...
    def initial_parameters(self, I: float, F_bar:list[float]):
        ...
        """
        self.I = I
        self.e_bar = self.ei(x=self.L, I=I, k=0)
        self.Fbar(*F_bar)
...
class PinnedPinned(ArbitrarySupport):
    ...
    def operator(self):
        """ """
        EI = self.E * self.I
        return (self.e_bar.e0
                * (self.e_bar.e4 / EI
                   - (self.e_bar.e2
                      + self.e_bar.Zeta * self.e_bar.e4)
                   / (self.G * As))
                - self.e_bar.e2**2)        
    #
    @property
    def V0(self) -> float:
        ...
        EI = self.E * self.I
        op = self.operator()
        return ((EI * self.e_bar.e0 * self.Fw_bar
                 + self.e_bar.e2 * self.FM_bar) / op)
    #
    @property
    def theta0(self) -> float:
        """ Slope"""
        EI = self.E * self.I
        op = self.operator()
        return (-1 * self.e_bar.e2 * self.Fw_bar -
                (self.e_bar.e4 / EI
                 - (self.e_bar.e2
                    + (self.e_bar.Zeta * self.e_bar.e4)
                    / (self.G * As))) / op)
...
class PinnedFixed(ArbitrarySupport):
    ...
    def operator(self):
        """ """
        EI = self.E * self.I
        return ((self.e_bar.e1 - self.e_bar.Eta * self.e_bar.e3)
                * (self.e_bar.e4 / EI
                   - (self.e_bar.e2
                      + (self.e_bar.Zeta * self.e_bar.e4))
                   / (self.G * As))
                - self.e_bar.e2 * self.e_bar.e3 / EI)
    #
    @property
    def V0(self) -> float:
        """ Shear force"""
        op = self.operator()
        return ((-1 * self.e_bar.e2 * self.Ftheta_bar
                 + (self.e_bar.e1
                    - self.e_bar.Eta * self.e_bar.e3) * self.Fw_bar)
                / op)
    #
    @property
    def theta0(self) -> float:
        ...
        op = self.operator()
        EI = self.E * self.I
        return ((-1 * self.Fw_bar * self.e_bar.e3 / EI
                 -self.Ftheta_bar
                 * (self.e_bar.e4 / EI
                    - (self.e_bar.e2 + self.e_bar.Zeta * self.e_bar.e4)
                    / (self.G * As)))
                / op)
#
@dataclass
class PinnedGuided(ArbitrarySupport):
    ...
    def operator(self):
        ...
        #EI = self.E * self.I
        return ((self.e_bar.e1 - self.e_bar.Eta * self.e_bar.e3)
                * (self.e_bar.e1 + self.e_bar.Zeta * self.e_bar.e3)
                +  self.e_bar.Lambda * self.e_bar.e3**2)
    #
    @property
    def V0(self) -> float:
        """ Shear force"""
        EI = self.E * self.I
        op = self.operator()
        return (((-1 * self.e_bar.Lambda * EI * self.e_bar.e3 * self.Ftheta_bar)
                - (self.e_bar.e1 - self.e_bar.Eta * self.e_bar.e3) * self.FV_bar)
                / op)
    #
    @property
    def theta0(self) -> float:
        ...
        EI = self.E * self.I
        op = self.operator()        
        return (((-1 * (self.e_bar.e1 + self.e_bar.Zeta * self.e_bar.e3) * self.Ftheta_bar)
                + self.e_bar.e3 / EI * self.FV_bar)
                / op)
#
#
@dataclass
class PinnedSpring(ArbitrarySupport):
    ...
    def operator(self):
        ...
        return ((A5 * self.e_bar.e2
                 + A6 * (self.e_bar.e4 - (self.e_bar.e2
                                          + self.e_bar.Zeta * self.e_bar.e4) * EI
                         / (self.G * As))))
    #
    @property
    def theta0(self) -> float:
        ...
        return ((A5 * self.Fw_bar
                 + A9 * (self.e_bar.e4
                         - (self.e_bar.e2 + self.e_bar.Zeta * self.e_bar.e4) * EI
                         / (self.G * As)))
                / op)
...
class FixedPinned(ArbitrarySupport):
    ...
    def operator(self):
        """ """
        EI = self.E * self.I
        return ((self.e_bar.e2 * self.e_bar.e3 / EI
                 - (self.e_bar.e4 / EI - (self.e_bar.e2 + self.e_bar.Zeta * self.e_bar.e4)
                    / (self.G * As))
                 * (self.e_bar.e1 - self.e_bar.Eta * self.e_bar.e3)))
    #
    @property
    def V0(self) -> float:
        """ Shear force"""
        EI = self.E * self.I
        op = self.operator()        
        return ((self.FM_bar * self.e_bar.e3 / EI
                 - self.Fw_bar * (self.e_bar.e1 - self.e_bar.Eta * self.e_bar.e3))
                / op)
    #
    @property
    def M0(self) -> float:
        ...
        EI = self.E * self.I
        op = self.operator()         
        return ((self.Fw_bar * self.e_bar.e2
                 + self.FM_bar * (self.e_bar.e4 / EI + (self.e_bar.e2 + self.e_bar.Zeta * self.e_bar.e4)
                                  / (self.G * As)))
                / op)
#
@dataclass
class FixedFixed(ArbitrarySupport):
    ...
    def operator(self):
        """ """
        EI = self.E * self.I
        return (self.e_bar.e3**2
                - (self.e_bar.e2 - self.e_bar.Eta * self.e_bar.e4)
                * (self.e_bar.e4 / EI - (self.e_bar.e2 + self.e_bar.Zeta * self.e_bar.e4)
                   / (self.G * As)))
    #
    @property
    def V0(self) -> float:
        ...
        EI = self.E * self.I
        op = self.operator()         
        return ((self.e_bar.e3 * self.Ftheta_bar
                 - (self.e_bar.e2 - self.e_bar.Eta * self.e_bar.e4) * self.Fw_bar)
                / op)
    ...
    def M0(self) -> float:
        ...
        EI = self.E * self.I
        op = self.operator()         
        return ((self.e_bar.e3 * self.Fw_bar
                 - (self.e_bar.e4 / EI - (self.e_bar.e2 + self.e_bar.Zeta * self.e_bar.e4)
                    / (self.G * As)))
                / op)
#
