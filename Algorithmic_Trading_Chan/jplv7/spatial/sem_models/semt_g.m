function results = semt_g(y,x,W,ndraw,nomit,prior)
% PURPOSE: Bayesian estimates of the spatial tobit error model
%          y = XB + u, u = rho*W + e
%          y = truncated, 0,y variable
%          e = N(0,1),  
%          B = N(c,T), 
%          1/sige = Gamma(nu,d0), 
%          rho = Uniform(rmin,rmax), or rho = beta(a1,a2); 
%-------------------------------------------------------------
% USAGE: results = semt_g(y,x,W,ndraw,nomit,prior)
...
% --------------------------------------------------------------
% SEE ALSO: (semt_gd, semt_gd2 demos) prt
% --------------------------------------------------------------
...
if n1 ~= n
error('semt_g: x-matrix contains wrong # of observations');
elseif n3 ~= n4
error('semt_g: W matrix is not square');
elseif n3~= n
error('semt_g: W matrix is not the same size at y,x');
end;
...
[nu,d0,rho,sige,rmin,rmax,detval,ldetflag,eflag,order,iter,novi_flag,c,T,cc,metflag,a1,a2,inform_flag] = semt_parse(prior,k);
...
[checkk,junk] = size(c);
if checkk ~= k
error('semt_g: prior means are wrong');
elseif junk ~= 1
error('semt_g: prior means are wrong');
end;
...
[checkk junk] = size(T);
if checkk ~= k
error('semt_g: prior bcov is wrong');
elseif junk ~= k
error('semt_g: prior bcov is wrong');
end;
...
hwait = waitbar(0,'semt\_g: MCMC sampling ...');
t0 = clock;                  
iter = 1;
acc = 0;
...
function [nu,d0,rho,sige,rmin,rmax,detval,ldetflag,eflag,order,iter,novi_flag,c,T,cc,metflag,a1,a2,inform_flag] = semt_parse(prior,k)
% PURPOSE: parses input arguments for semt models
% ---------------------------------------------------
...
if nf > 0
 for i=1:nf
    ...
    elseif strcmp(fields{i},'lflag')
        ...
        error('semt_g: unrecognizable lflag value on input');
        end;
...
elseif ldetflag == -1 % the user fed down a detval matrix
    time1 = 0;
        ...
        if detval == 0
            error('semt_g: wrgon lndet input argument');
        end;
        [n1,n2] = size(detval);
        if n2 ~= 2
            error('semt_g: wrong sized lndet input argument');
        elseif n1 == 1
            error('semt_g: wrong sized lndet input argument');
        end;          
end;
