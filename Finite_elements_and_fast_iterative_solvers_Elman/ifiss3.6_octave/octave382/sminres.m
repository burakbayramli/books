function [ x, istop, itn, rnorm, Arnorm, Anorm, Acond, ynorm, resvec ] = ...
           sminres( A, b, M, shift, show, check, itnlim, rtol, varargin)
%SMINRES  preconditioned MINRES
%        [ x, iflag, itn, rnorm, Arnorm, Anorm, Acond, ynorm, resvec ] = ...
%          sminres( A, b, M, shift, show, check, itnlim, rtol, varargin )
%
% sminres solves the n x n system of linear equations Ax = b
...
%
% If shift ~= 0, sminres really solves (A - shift*I)x = b
% (or the corresponding least-squares problem if shift is an
% eigenvalue of A).
%
% When M = C*C' exists, sminres implicitly solves the system
%
...
% 26 Jan 2010: Reconfigured by David Silvester for the Octave 3.2.3
%              version of IFISS 3.0:
%              - test1 sequence returned in the output parameter resvec
%              - varargin input added and passed to the 
%                preconditioning routine minresxxxM 
...
if show
   ...
   fprintf('\n                      IFISS 3.0 implementation on 26 Jan 2010')
...
elseif isa(A,'function_handle')
  ...
  error('sminres','A must be a matrix or a function handle')
end
...
if precon
  ...
  elseif isa(M,'function_handle')
    ...
    error('sminres','M must be a matrix or a function handle')
  end
end
...
y     = b;
r1    = b;
if precon, y = minresxxxM( M,b,varargin{:});  %%%bologna
end
beta1 = b'*y;
...
if beta1> 0
  ...
  if check & precon
    r2     = minresxxxM( M,y, varargin{:});  %%%bologna
    s      = y' *y;
    t      = r1'*r2;
    z      = abs(s-t);
    epsa   = (s+eps)*eps^(1/3);
...
if ~done                              % k = itn = 1 first time through
  while itn < itnlim
    ...
    r1     = r2;
    r2     = y;
    if precon,  y = minresxxxM( M,r2, varargin{:});  %%%bologna
    end
...
function y = minresxxxM( M,x, varargin)   %%%bologna
