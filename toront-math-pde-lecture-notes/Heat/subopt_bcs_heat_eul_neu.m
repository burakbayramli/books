% [u,err,x,t] = subopt_bcs_heat_eul_neu(t_0,t_f,D,L,M,N)
%
...
function [u,err,x,t] = subopt_bcs_heat_eul_neu(t_0,t_f,D,L,M,N)
...
%
% I approximate u_x(0,t) = 0 by (u_old(0)-u_old(1))/dx = 0 hence 
% I'll take u_old(0) = u_old(1). 
%
%    u_new(1) = u_old(1) + r*(u_old(2)-2*u_old(1)+u_old(1))     
%    u_new(N) = u_old(N) + r*(u_old(N-1)-2*u_old(N)+u_old(N-1))     


for j=1:M
  u(1,j+1) = u(1,j) + r*(u(2,j)-2*u(1,j)+u(1,j));
  for i=2:N
