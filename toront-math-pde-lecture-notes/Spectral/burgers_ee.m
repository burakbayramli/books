% [u,x,t,kk,amp] = burgers_ee(u0,D,t_0,t_f,M,N)
%
...
% space.  I do explicit euler time-stepping.
...
function [u,x,t,kk,amp] = burgers_ee(u0,D,t_0,t_f,M,N)
...
% so in Fourier space I have the ODE.  I use explicit Euler
% to timestep.
%
%     d/dt hat{u^n}_k = -  hat{g^n}_k - D k^2 hat{u^n}_k
%
%     hat{u^{n+1}}_k - hat{u^n}_k = dt*( - hat{g^n}_k - D k^2 hat{u^{n}}_k)
% 
%     hat{u^{n+1}}_k  = hat{u^n}_k - D k^2 dt hat{u^n}_k - dt hat{g^n}_k 
% 
%     hat{u^{n+1}}_k = (1 - D k^2 dt)*hat{u^n}_k - dt hat{g^n}_k
%
U_now = fft(u(:,1));
for j=1:M
    G = convolve(U_now);
    ...
    % the nonzero modes
    for k = 1:floor(N/2)-1
        U_new(k+1) = (1 - D*k^2*dt)*U_now(k+1) - dt*G(k+1);
        U_new(N-k+1) = (1 - D*k^2*dt)*U_now(N-k+1) - dt*G(N-k+1);
    end
