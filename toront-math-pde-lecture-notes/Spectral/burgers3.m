% [u,x,t,kk,amp] = burgers3(u0,D,t_0,t_f,M,N)
%
...
function [u,x,t,kk,amp] = burgers3(u0,D,t_0,t_f,M,N)
...
% I first take one time step:
%
% At time t_0, I use convolve.m to compute the nonlinearity u^0 u^0_x.  It
% gives me the fourier transform of u^0 u^0_x.  Let's call that g^0.  And
...
% leads to
%
%     hat{u^1}_k = exp{- D k^2 dt}( hat{u^0}_k - dt hat{g^0}_k )
%
U_now = fft(u(:,1));
G = convolve(U_now);
% the zero mode:
