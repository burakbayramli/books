...
% to timestep.
%
% d/dt hat{u^n}_k = hat{g^n}_k - k^2 hat{u^n}_k 
%
% hat{u^{n+1}}_k - hat{u^n}_k = dt*( hat{g^n}_k + dt*( - k^2 ) hat{u^{n+1}}_k
% 
% hat{u^{n+1}}_k + k^2 dt hat{u^{n+1}}_k = hat{u^n}_k + dt hat{g^n}_k 
% 
% hat{u^{n+1}}_k = (hat{u^n}_k + dt hat{g^n}_k)/(1 + k^2 dt) 
%
U_now = fft(u(:,1))';
for j=1:M
    G = convolve(U_now);
    ...
    for k = 1:floor(N/2)-1
        U_new(k+1) = (U_now(k+1) + dt*G(k+1))/(1 + k^2*dt);
        U_new(N-k+1) = (U_now(N-k+1) + dt*G(N-k+1))/(1 + k^2*dt);
    end
...
% of 1/100 u^3 u_x + 10 H[u_x].  
%
function V = convolve(u)

N = length(u);

% I want to create 1/100 u^3 u_x + 10 H[u_x].  
% So I create
...
% here I create something which will be Hilbert(u_x) when IFFTed
w(1) = 0;
for k=1:floor(N/2)-1
    w(k+1) = k*u(k+1);
    w(N-k+1) = k*u(N-k+1);
