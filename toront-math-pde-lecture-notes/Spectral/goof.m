% [u,x,t,kk,amp] = burgers2b(u0,D,t_0,t_f,M,N)
%
...
for j=1:M
    G = convolve(U_now);
    % the zero mode:
    U_new(1) = U_now(1) + dt*G(1)/100;
    for k = 1:floor(N/2)-1
        U_new(k+1) = (U_now(k+1) + dt*G(k+1)/100)/(1 + (k^2-10*abs(k))*dt);
        U_new(N-k+1) = (U_now(N-k+1) + dt*G(N-k+1)/100)/(1 + (k^2-10*abs(k))*dt);
    end
