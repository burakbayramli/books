% [u,x,t,kk,amp,M,H] = split_schroedinger3(u0,K,t_0,t_f,dt,NPrint,N)
%
...
% space and a Strang splitting method for the time stepping,followed by 
% Richardson extrapolation for acceleration.  t_0 is the 
...
%
function [u,x,t,kk,amp,M,H] = split_schroedinger3(u0,K,t_0,t_f,dt,NPrint,N)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% I'm doing a Strang splitting here...
%
function u_now = tstep(u_now,K,dt,N);

% first take one step of size dt/2 of the equation u_t = i u_xx
v = fft(u_now);
ii = 1:N/2;
k = (ii-1);
...
% now take one step of size dt of u_t = i K |u|^2 u,
% using v as our initial data
v = exp(i*K*abs(v).^2*dt).*v;

% now take one step of size dt/2 of u_t = i u_xx
v = fft(v);
ii = 1:N/2;
k = (ii-1);
v(ii) = exp(-i*k.^2*dt/2).*v(ii);    
