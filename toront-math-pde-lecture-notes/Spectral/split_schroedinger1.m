% [u,x,t,kk,amp,M,H] = split_schroedinger1(u0,K,t_0,t_f,dt,NPrint,N)
%
...
% A splitting method is used for the time stepping.  t_0 is the 
...
% and H which is the hamiltonian \int |u_x|^2 - K/2 |u|^4

function [u,x,t,kk,amp,M,H] = split_schroedinger(u0,K,t_0,t_f,dt,NPrint,N)
...
% first take one time-step of the equation u_t = i u_xx
v = fft(u_now);
ii = 1:N/2;
k = (ii-1);
v(ii) = exp(-i*k.^2*dt).*v(ii);    
ii = N/2+2:N;
k = (ii-N-1);
v(ii) = exp(-i*k.^2*dt).*v(ii);    
v = ifft(v);

% now take one time-step of u_t = i K |u|^2 u,
% using v as our initial data
u_now = exp(i*K*abs(v).^2*dt).*v;
