...
def runtest(dir, file, description, results):
    path = os.path.join(dir,file)
    print 'running', path, '\n   ' + description
    # (note: joining dir and file may fail on Windows if directories in
    # the dir path contain blanks, then it's better to add dir to PATH)
    t0 = os.times()
    os.system(path + ' datatrans.tmp tmp.1')
    t1 = os.times()
