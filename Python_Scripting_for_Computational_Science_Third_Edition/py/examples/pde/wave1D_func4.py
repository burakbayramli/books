#!/usr/bin/env python
"""
As wave1D_func1.py, but the scheme + initial and boundary conditions
are extracted to a separate functions.
One can in a similar way let the class in wave1D_class.py do just
the administrative work and call up separate functions to do the
core numerics.
"""
...
def ic_scalar(u, um, x, I, U_0, U_L, f, n, C2, dt):
    t = 0
    for i in iseq(0,n):
        u[i] = I(x[i])
    for i in iseq(1,n-1):
...
def scheme_scalar(up, u, um, C2, f, x, t_old, n, dt):
...
def scheme_vec(up, u, um, C2, f, x, t_old, n, dt):
    up[1:n] = - um[1:n] + 2*u[1:n] + \
              C2*(u[0:n-1] - 2*u[1:n] + u[2:n+1]) + \
              dt*dt*f(x[1:n], t_old)
    return up
    
def bc_scalar(up, U_0, U_L, t, n):
...
def solver(I, f, c, U_0, U_L, L, n, dt, tstop,
    ...
    t = 0.0
    u, um = ic_scalar(u, um, x, I, U_0, U_L, f, n, C2, dt)
    ...
    while t <= tstop:
        ...
        if version == 'scalar':
            up = scheme_scalar(up, u, um, C2, f, x, t_old, n, dt)
        elif version == 'vectorized':
            up = scheme_vec   (up, u, um, C2, f, x, t_old, n, dt)
        else:
            raise ValueError, 'version=%s' % version
            
        # insert boundary conditions:
        up = bc_scalar(up, U_0, U_L, t, n)
        if user_action is not None:
            user_action(up, x, t)
