...
c     from NASA TM X-3069, September 1974

c     by Ladson, C.L., and Brooks, C.W., Jr.
c     ported to the Mac by W.H. Mason, March 1991
 
      DIMENSION XU(201),   XL(201),  YU(201),    YL(201),     
     1          XT(201),   YT(201),  YTP(201),   YTPP(201),
     2          PHI(201),  EPS(201), PSI(201),   KF6XA(10),
     3          CLI(10),   A(10),    TANTH0(10), YCMB(10), 
     4          TANTH(10), YCP2(10) 

      character*10 NAME(8),SERIET,SERIEC
      character*20 filenm
...
C       TOC - T/C, THICKNESS, RLE - LEADING EDGE RADIUS, XM - X(YMAX)/CHOR
C       DX  - INTERVAL/CHORD, CHD - CHORD IN DESIRED UNITS

C     READ SPECIFICATIONS FOR PROFILE

   30 READ (2,440) NAME,SERIET,SERIEC,TOC,RLE,CHD,CLI(1),A(1),CMBNMR
      ICKY     = CMBNMR
      IF (ICKY.LT.1) ICKY=1
      DO 20 I  = 1,10
      KF6XA(I) = 0
...
C     READ ADDITIONAL ADDITIVE CAMBERLINES

      IF (ICKY.GT.1) READ (2,450) (CLI(J),A(J),J=2,ICKY)
      ICKYP   = ICKY+1
      DO 60 J = ICKYP,10
      CLI(J)  = 0.0
      A(J)    = 0.0
...
C     PRINT INPUTS
      ...
      write(6,500) NAME
      write(6,580) SERIET,SERIEC
      write(6,460) TOC,RLE,CHD,(CLI(J),J=1,ICKY)
      write(6,470) (A(J),J=1,ICKY)

C       COLUMN HEADING FORMATS
C       SLOPE OF CAMBERLINE AT ORIGIN, TANTH0
   ...
   70 L      = L+1
      ...
      V      = -(A1-U)/ABS(A1-U)
      OMXL   = (1.-U)*ALOG(1.-U)
      AMXL   = (A1-U)*ALOG(ABS(A1-U))
      OMXL1  = -ALOG(1.-U)-1.
      AMXL1  = -ALOG(ABS(A1-U))+V
      OMXL2  = 1./(1.-U)
      AMXL2  = -V/ABS(A1-U)
      IF (A1.LT.E.OR.ABS(1.-A1).LT.E) GO TO 80
      G      = -(A1*A1*(.5*ALOG(A1)-0.25)+0.25)/(1.-A1)
      Q      = 1.0
      H  = (0.5*(1.-A1)**2*ALOG(1.-A1)-0.25*(1.-A1)**2)/(1.-A1)+G
      Z  = .5*(A1-U)*AMXL-.5*(1.-U)*OMXL-.25*(A1-U)**2+.25*(1.-U)**2
      Z1 = .5*((A1-U)*AMXL1-AMXL-(1.-U)*OMXL1+OMXL+(A1-U)-(1.-U))
      Z2 = .5*(A1-U)*AMXL2-AMXL1-.5*(1.-U)*OMXL2+OMXL1
   80 CONTINUE
      IF (A1.LT.E) GO TO 90
      IF (ABS(A1-1.).LT.E) GO TO 100
   90 H  = -.5
      Q  = 1.0
  ...
  100 H  = 0.0
      ...
      Z1 = -OMXL1
      GO TO 110
  110 TANTH0(L)=CLI1*(Z1/(1.-Q*A1)-1.-ALOG(U)-H)/PI/(A1+1.)/2.0
      IF (ICKY.GT.1.AND.L.LT.ICKY) GO TO 70
      IF (ICKY.EQ.1) GO TO 130
      DO 120 J = 2,ICKY
  120 TANTH0(1)  =TANTH0(1) + TANTH0(J)
  130 CONTINUE
...
C       SELECT SERIES
      ...
      IF (SERIET.EQ.10H        63) CALL PEP63 (PHI,EPS)
      IF (SERIET.EQ.10H        64) CALL PEP64 (PHI,EPS)
      IF (SERIET.EQ.10H        65) CALL PEP65 (PHI,EPS)
      IF (SERIET.EQ.10H        66) CALL PEP66 (PHI,EPS)
      IF (SERIET.EQ.10H        67) CALL PEP67 (PHI,EPS)
      IF (SERIET.EQ.10H        63) CALL PPS63 (PHI,PSI)
      IF (SERIET.EQ.10H        64) CALL PPS64 (PHI,PSI)
      IF (SERIET.EQ.10H        65) CALL PPS65 (PHI,PSI)
      IF (SERIET.EQ.10H        66) CALL PPS66 (PHI,PSI)
      IF (SERIET.EQ.10H        67) CALL PPS67 (PHI,PSI)
      IF (SERIET.EQ.10H       63A) CALL PEP63A (PHI,EPS)
      IF (SERIET.EQ.10H       64A) CALL PEP64A (PHI,EPS)
      IF (SERIET.EQ.10H       65A) CALL PEP65A (PHI,EPS)
      IF (SERIET.EQ.10H       63A) CALL PPS63A (PHI,PSI)
      IF (SERIET.EQ.10H       64A) CALL PPS64A (PHI,PSI)
      IF (SERIET.EQ.10H       65A) CALL PPS65A (PHI,PSI)
      RAT    = 1.0
      IT     =   0
      ACRAT  = 1.0
...
C       LOOP START FOR THICKNESS ITERATION
  ...
  160 CONTINUE
      XTP      = 1.0
      DO 170 J = 2,201
      YTP(J)   = DIF(J,5,201,XT,YT)
      IF (J.LT.3) GO TO 170
      IF (YTP(J).LT.0.0.AND.YTP(J-1).GT.0.0) XTP=XT(J-1)+YTP(J-1)*
     1  (XT(J)-XT(J-1))/(YTP(J-1)-YTP(J))
  170 CONTINUE
      YTP(1)   = 10.**6
      CALL FTLUP (XTP,YM,2,201,XT,YT)
      DO 180 J = 2,201
      YTPP(J)  = DIF(J,5,201,XT,YTP)
  180 CONTINUE
      YTPP(1)  = 10.**6
      XO       = XT(1)
      XL201    = XT(201)
      TR       = 2.*YM/(XL201-XO)
      IF (TOC.LE.E) GO TO 190

C       TEST THICKNESS

      RAT     = TOC/TR
      IF (ABS(RAT-1.0).GT.0.0001.AND.IT.LE.10) GO TO 150
  190 SF      = RAT
      IF (TOC.LT.E) SF=0.0
      IF (I.GT.1)   GO TO 210
      DO 200 J = 1,201
...
C       SCALE LINEARLY TO EXACT THICKNESS
  ...
  210 CONTINUE
      ...
      XYM     = (XYM-XO)/(XL201-XO)
      XL(1)   = 0.0
      IF (TOC.LE.E) GO TO 230

C       FIT TILTED ELLIPSE AT ELEVENTH PROFILE POINT

      CN = 2.*YTP(11)-YT(11)/XT(11)+0.1
      AN = XT(11)*(YTP(11)*XT(11)-YT(11))/
     1            (XT(11)*(2.*YTP(11)-CN)-YT(11))
      BN       = SQRT((YT(11)-CN*XT(11))**2/(1.-(XT(11)-AN)**2/AN**2))
      DO 220 J = 1,10
      YT(J)    = BN*SQRT(1.-(XT(J)-AN)**2/AN**2)+CN*XT(J)
      IF (XT(J).LE.E) GO TO 220
      YTP(J)   = BN**2*(AN-XT(J))/AN**2/(YT(J)-CN*XT(J))+CN
      YTPP(J)  = -BN**4/AN**2/(YT(J)-CN*XT(J))**3
  220 CONTINUE
      RNP      = BN**2/AN
      IF (I.EQ.1) write(6,520) XYM,YMAX,XTP,YM,XT(11),YT(11),YTP(11),
     1                         RNP,RAT,ACRAT,IT
  230 CONTINUE
      X       = 0.0
      ALI     = ABS(CLI(1))

C       PRINT UNCAMBERED COLUMN HEADINGS AND ORIGIN POINT

      IF (ALI.LE.E.AND.ICKY.EQ.1) write(6,480)
      IF (ALI.LE.E.AND.ICKY.EQ.1) write(6,550) X,Y,YP,YPP,XC,YC

C       PRINT CAMBERED COLUMN HEADINGS AND ORIGIN POINT

      IF (ALI.GT.E.OR.ICKY.GT.1) write(6,490)
      IF (ALI.GT.E.OR.ICKY.GT.1) write(6,540) X,XU(I),YU(I),XUC,YUC,
     1                                  YUP,XL(I),YL(I),XLC,YLC,YLP
      X     = 0.00025
      XL(1) = 0.0
  240 CONTINUE
...
C  INTERPOLATE FOR THICKNESS AND DERIVATIVES AT DESIRED VALUES OF X
      ...
      CALL FTLUP (X,Y,2,201,XT,YT)
      CALL FTLUP (X,YP,2,201,XT,YTP)
      CALL FTLUP (X,YPP,2,201,XT,YTPP)
...
C       COMPUTE CAMBERLINE
  ...
  250 L      = L+1
      ...
      XLL    = X*ALOG(X)
      Q      = 1.0
      IF (ABS(1.-A1).LT.E.AND.ABS(1.-X).LT.E) GO TO 300
      IF (A1.LT.E.AND.(1.-X).LT.E)            GO TO 310
      IF (ABS(A1-X).LT.E)                     GO TO 260
      IF (ABS(1.-X).LT.E)                     GO TO 280
      IF (ABS(A1-1.).LT.E)                    GO TO 290
      V     = -(A1-X)/ABS(A1-X)
      OMXL  =  (1.-X)*ALOG(1.-X)
      AMXL  =  (A1-X)*ALOG(ABS(A1-X))
      OMXL1 = -ALOG(1.-X)-1.
      AMXL1 = -ALOG(ABS(A1-X))-1.
      OMXL2 =  1./(1.-X)
      AMXL2 =  1./(A1-X)
      Z  = .5*(A1-X)*AMXL-.5*(1.-X)*OMXL-.25*(A1-X)**2+.25*(1.-X)**2
      Z1 = .5*((A1-X)*AMXL1-AMXL-(1.-X)*OMXL1+OMXL+(A1-X)-(1.-X))
      Z2 = .5*(A1-X)*AMXL2-AMXL1-.5*(1.-X)*OMXL2+OMXL1
      IF (A1.LE.E) GO TO 270
      G  = -(A1*A1*(.5*ALOG(A1)-0.25)+0.25)/(1.-A1)
      H  =  (0.5*(1.-A1)**2*ALOG(1.-A1)-0.25*(1.-A1)**2)/(1.-A1)+G
      GO TO 320
  260 Z  = -.5*(1.-X)**2*ALOG(1.-X)+0.25*(1.-X)**2
      Z1 = -.5*(1.-X)*(-ALOG(1.-X)-1.)+.5*(1.-X)*ALOG(1.-X)-.5*(1.-X)
      Z2 = -ALOG(1.-X)-0.5
      G  = -(A1**2*(.5*ALOG(A1)-0.25)+0.25)/(1.-A1)
      H  = (0.5*(1.-A1)**2*ALOG(1.-A1)-0.25*(1.-A1)**2)/(1.-A1)+G
      GO TO 320
  270 G  = -.25
      H  = -.50
  ...
  280 CONTINUE
      G  = -(A1**2*(.5*ALOG(A1)-0.25)+0.25)/(1.-A1)
      H  =  (0.5*(1.-A1)**2*ALOG(1.-A1)-0.25*(1.-A1)**2)/(1.-A1)+G
      Z  =  .5*(A1-1.)**2*ALOG(ABS(A1-1.))-0.25*(A1-1.)**2
      Z1 = -(A1-1.)*ALOG(ABS(A1-1.))
      Z2 = -10.**10
      GO TO 320
  290 G  = 0.0
      H  = G
  ...
  310 G  = -.25
      ...
      Z2 = -10.**10
      GO TO 320
  320 YCMB(L)  = CLI1*(Z/(1.-Q*A1)-XLL+G-H*X)/PI/(A1+1.)/2.
      XSV      = X
      IF (X.LT.0.005) X=0.005
      TANTH(L) = CLI1*(Z1/(1.-Q*A1)-1.-ALOG(X)-H)/PI/(A1+1.)/2.0
      X        = XSV
      IF (KF6XA(L).EQ.1) TANTH(L)=-5.0
      IF (X.GT.0.005) GO TO 330
      YCP2(L)  = 0.0
      GO TO 350
  330 CONTINUE
      IF (ABS(1.-X).GT.E) GO TO 340
      YCP2(L) = 1./E
      GO TO 350
  340 PIA     = PI*(A1+1.)*2.
      YCP2(L) = CLI1*(Z2/(1.-Q*A1)-1./X)/PIA
  350 CONTINUE

C       MODIFIED CAMBERLINE OPTION

      IF (SERIEC.EQ.10H       63A.OR.
     1    SERIEC.EQ.10H       64A.OR.
     2    SERIEC.EQ.10H       65A) GO TO 360
      GO TO 380
  360 YCMB(L)  =  YCMB(L)*0.97948
      TANTH(L) = TANTH(L)*0.97948
      YCP2(L)  =  YCP2(L)*0.97948
      a1       = A(1)
      cli1     = cli(1)
      IF(ABS(A1-.8).LT.E.OR.CLI1.LT.E) GO TO 370
      write(6,530)
      READ (5,500) NPWIPE
      GO TO 50
  370 CONTINUE
      IF (TANTH(L).LE.-.24521*CLI1) YCMB(L)  =  0.24521*CLI1*(1.-X)
      IF (TANTH(L).LE.-.24521*CLI1) YCP2(L)  =  0.0
      IF (TANTH(L).LE.-.24521*CLI1) TANTH(L) = -0.24521*CLI1
      IF (TANTH(L).LE.-.24521*CLI1) KF6XA(L) =  1
  380 CONTINUE
      IF (ICKY.GT.1.AND.L.LT.ICKY) GO TO 250
      IF (ICKY.EQ.1)               GO TO 400
...
C       COMBINE THICKNESS DISTRIBUTION AND CAMBERLINE
...
C       MULTIPLY BY CHORD
      ...
      IF (ALI.LE.E.AND.ICKY.EQ.1) GO TO 420
...
C       FIND LOCAL SLOPE OF CAMBERED PROFILE
      ...
      IF (TOC.LT.E) GO TO 420

  420 CONTINUE

C       FIND X INCREMENT

      IF (X.LE.0.09750) FRAC = 0.250
      IF (X.LE.0.01225) FRAC = 0.025

C       PRINT PROFILE IN APPROPRIATE FORMAT
...
C       INCREMENT X AND RETURN TO START OF X LOOP
      ...
      X    = X + FRAC*DX
      FRAC = 1.0
      IF (X.LE.1.0) GO TO 140
C
      call stdout(xu,yu,xl,yl,name,i)

   50 stop
C
  424 format(/2x,'enter name of input data file')
  428 format(a20)
  430 FORMAT (I3/(8F10.0))
  440 FORMAT (8A10/2A10/6F10.0)
  450 FORMAT (8F10.0)
  460 FORMAT (2x,' TOC = ',F10.6,2x,' RLE = ',F10.6,
     1        2x,' CHD = ',F10.6/2x,' CLI = ',10F10.6)
  470 FORMAT (2x,' A   = ',10F10.6/)
  480 FORMAT (9X,3HX/C,10X,3HY/C,8X,5HDY/DX,6X,7HD2Y/DX2,22X,
     1        1HX,12X,1HY/)
  490 FORMAT (//29x,'UPPER SURFACE VALUES',
     1          33x,'LOWER SURFACE VALUES' /'  UNCAMBERED',/5X,
     2    3HX/C,7X,4HXU/C,7X,4HYU/C,8X,2HXU,9X,2HYU,6X,7HDYU/DXU,8X,
     3    4HXL/C,7X,4HYL/C,8X,2HXL,9X,2HYL,6X,7HDYL/DXL)
  500 FORMAT(/2x,8A10/)
  510 FORMAT (2x,' RAT(',I2,') = ',F10.5)
  520 FORMAT (/3x,'PEAK IS AT X/C =',F10.6/
     1         3x,'MAXIMUM Y/C IS  ',F10.6/
     2         3x,'SLOPE CHANGES SIGN AT X/C, Y/C = ',2F10.6/
     3         3x,'X/C FIT OF ELLIPSE   ',F10.6/
     4         3x,'Y/C FIT OF ELLIPSE   ',F10.6/
     5         3x,'SLOPE FIT OF ELLIPSE ',F10.6/
     6 3x,'RADIUS AT ORIGIN OF ELLIPSE THRU XT(11)/C,YT(11)/C',
     7 F10.6/  3x,'RATIO OF T/C INPUT TO T/C COMPUTED IS ',F10.6/
     8         3x,'CUMULATIVE SCALING OF EPS,PSI ',F10.6/
     9         3x,'NUMBER OF ITERATIONS=',I3)
  530 FORMAT (53H  MODIFIED CAMBER LINE OPTION ALLOWED ONLY FOR A=0.8 )
  540 FORMAT (1x,F10.5,5F11.6,2X,5F11.6)
  550 FORMAT (4F13.6,10X,2F13.6)
  560 FORMAT (8A10/(8F10.5))
  580 format (/'   SERIET =',A10,'   SERIEC = ',A10/)
  590 format(/5x,'Program LADSON            version from W.H. Mason'//
     1        5x,'Approximated Ordinates for NACA 6 and 6A Airfoils'/
     2        5x,'NASA TM X-3069, September 1974'/)

      END

      subroutine stdout(xu,yu,xl,yl,name,imax)

c     output to a file for use as input to other codes
...
C
      write(6,424)
      read (5,428) filenm
      open(unit = 3, file = filenm, status = 'new')

      fnu    = 80.
      fnl    = 80.

c     adjust trailing edge ordinates

      xute   = 1.0
      au     = (yu(174) - yu(175))/(xu(174) - xu(175))
      yute   = yu(175) + au*(xute - xu(175))

      xlte   = 1.0
      al     = (yl(174) - yl(175))/(xl(174) - xl(175))
      ylte   = yl(175) + al*(xlte - xl(175))

      if(ylte .gt. yute) then
                         yte   = (yute + ylte)/2.0
                         yute  = yte
                         ylte  = yte
                         endif
...
c     write out the file
      ...
      do 10 i = 1,25,3
   10 write(3,20)         XU(I),   YU(I)
      do 12 i = 29,49,4
   12 write(3,20)         XU(I),   YU(I)
      do 14 i = 52,86,2
   14 write(3,20)         XU(I),   YU(I)
      write(3,20)         XU(87),  YU(87)
      do 16 i = 88,174,2
   16 write(3,20)         XU(I),   YU(I)
      write(3,20)         XU(175), YU(175)
      write(3,20)         xute,    yute


      write(3,50)

      do 60 i = 1,25,3
   60 write(3,20)         XL(I),   YL(I)
      do 62 i = 29,49,4
   62 write(3,20)         XL(I),   YL(I)
      do 64 i = 52,86,2
   64 write(3,20)         XL(I),   YL(I)
      write(3,20)         XL(87),  YL(87)
      do 66 i = 88,174,2
   66 write(3,20)         XL(I),   YL(I)
      write(3,20)         XL(175), YL(175)
      write(3,20)         xlte,    ylte

      return

   20 format(2f10.6)
   30 format(2x,'UPPER SURFACE')
   50 format(2x,'LOWER SURFACE')
  100 format(8a10)
  424 format(/2x,'enter name of output data file')
  428 format(a20)
...
C
C     DIF=O17770000000000000000
C   FOR NOS SYSTEM, PREVIOUS CARD MUST BE REPLACED BY:
cwhm  DIF=17770000000000000000B
      IF (M.LT.1) RETURN
      N=2*M+1
...
c     first order
c
      y = (vard(i)*(vari(i+1)-x)-vard(i+1)*(vari(i)-x))/
     1    (vari(i+1)-vari(i))
...
170   format(1h1,' table below out of order for ftlup  at position ',
     1i5,/' x table is stored in location  ',i6//8g15.8)
      ...
      SUBROUTINE PEP63(PHI,EPS)
      DIMENSION PHI(1),EPS(1)
      DIMENSION PHILD(201),EPSLD(201),PANDE(201,2)
      EQUIVALENCE (PHILD(1),PANDE(1,1)),(EPSLD(1),PANDE(1,2))
      DATA ((PANDE(I,J),J=1,2),I=1,25 )/
     ...
     1    .06602,   1.53938,    .06681,   1.55509,    .06757/
      DATA ((PANDE(I,J),J=1,2),I=101,125)/
     ...
     1    .07891,   1.93208,    .07908,   1.94779,    .07922/
      DATA ((PANDE(I,J),J=1,2),I=126,150)/
     ...
     1    .07293,   2.32479,    .07222,   2.34049,    .07148/
      DATA ((PANDE(I,J),J=1,2),I=151,175)/
     ...
     1    .04549,   2.71749,    .04404,   2.73320,    .04256/
      DATA ((PANDE(I,J),J=1,2),I=176,201)/
  ...
  201 EPS(J)=EPSLD(J)
      ...
      SUBROUTINE PEP63A(PHI,EPS)
      DIMENSION PHI(1),EPS(1)
      DIMENSION PHILD(251),EPSLD(251),PANDE(251,2)
      EQUIVALENCE (PHILD(1),PANDE(1,1)),(EPSLD(1),PANDE(1,2))
      DATA ((PANDE(I,J),J=1,2),I=1,25 )/
     ...
     1    .06675,   1.86414,    .06689,   1.87461,    .06702/
      DATA ((PANDE(I,J),J=1,2),I=176,200)/
     ...
     1    .06613,   2.13284,    .06593,   2.14392,    .06571/
      DATA ((PANDE(I,J),J=1,2),I=201,225)/
     ...
     1    .05583,   2.44304,    .05517,   2.45755,    .05448/
      DATA ((PANDE(I,J),J=1,2),I=226,251)/
  ...
  201 CALL FTLUP(PHI(J),EPS(J),2,251,PHILD,EPSLD)
      RETURN
      END

      SUBROUTINE PEP64(PHI,EPS)
      DIMENSION PHI(1),EPS(1)
      DIMENSION PHILD(201),EPSLD(201),PANDE(201,2)
      EQUIVALENCE (PHILD(1),PANDE(1,1)),(EPSLD(1),PANDE(1,2))
      DATA ((PANDE(I,J),J=1,2),I=1,25 )/
  ...
  201 CALL FTLUP(PHI(J),EPS(J),2,251,PHILD,EPSLD)
      ...
      SUBROUTINE PPS63(PHI,PSI)
      DIMENSION PHI(1),PSI(1)
      DIMENSION PHILD(201),PSILD(201),PANDE(201,2)
      EQUIVALENCE (PHILD(1),PANDE(1,1)),(PSILD(1),PANDE(1,2))
      DATA ((PANDE(I,J),J=1,2),I=1,25 )/
     ...
     1   3.14159,   0.00000/
      DO 202 J=1,201
      CALL FTLUP(PHI(J),PSI(J),2,201,PHILD,PSILD)
  202 CONTINUE
      RETURN
      END

      SUBROUTINE PPS63A(PHI,PSI)
      DIMENSION PHI(1),PSI(1)
      DIMENSION PHILD(251),PSILD(251),PANDE(251,2)
      EQUIVALENCE (PHILD(1),PANDE(1,1)),(PSILD(1),PANDE(1,2))
      DATA ((PANDE(I,J),J=1,2),I=1,25 )/
     ...
     1   3.14159,    .02243/
      DO 202 J=1,201
      CALL FTLUP(PHI(J),PSI(J),2,251,PHILD,PSILD)
  202 CONTINUE
      RETURN
      END

      SUBROUTINE PPS64(PHI,PSI)
      DIMENSION PHI(1),PSI(1)
      DIMENSION PHILD(201),PSILD(201),PANDE(201,2)
      EQUIVALENCE (PHILD(1),PANDE(1,1)),(PSILD(1),PANDE(1,2))
      DATA ((PANDE(I,J),J=1,2),I=1,25 )/
