function varargout = fAESURFZ(varargin)
func = varargin{1};
switch func
    case 'readline'
        varargout{1} = readline(varargin{2},varargin{3},varargin{4});
        
        
    case 'minRows'
        varargout{1} = 1;
    case 'maxRows'
        varargout{1} = 1;
    otherwise
        error('invalid input option')
end

end


function placeHolder = readline(row,placeHolder,C)

fields = {'AESURFZ','LABEL','TYPE','CID','SETK','SETG','ACTID',[],[]};
dataType = {'str','str','str','int','int','int','int',[],[]};
default = {Inf,[],Inf,0,[],Inf,Inf,Inf,Inf};
placeHolder =  readEntryLine(fields,dataType,default,row,placeHolder,C) ;

end