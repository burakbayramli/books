%% VoLAre Example 1c: Full-span swept wing analysis using coarse mesh.
%  This example is used to check the alternate full-span boundary condition
%  option.  
%
% Anthony Ricciardi
% August 2017 
...
% o Mesh and single verification case from Bertin [2] 
% o Second verification case produced using AVL 3.27 [3]
% 
...
% [2] Bertin, J., ``Aerodynamics for Engineers 4th Edition'', Prentice 
%     Hall, 2001.
% [3] Drela, M. and Youngren, H., ``AVL User Primer'', Accessed 2013.
...
%% Select input file
filename = 'coarseMesh_fullSpan.inp';
...
avl = readtable('AVL_coarse_mesh.csv','HeaderLines',1);
bertin = readtable('Bertin_coarse_mesh.csv','HeaderLines',1);
legendStr = {'VoLAre','AVL 3.27','Bertin','Wind Tunnel'};
...
figure(1)
plot([0,alpha],[0,[C.L]],'o-',avl.AoA_deg,avl.C_L,'o-',bertin.AoA_deg,bertin.C_L,'-',wb.AoA_deg,wb.C_L,'o-')
...
ylabel('C_D'); xlabel('\alpha (deg.)'); legend(legendStr([1:2,4]),'location','northwest');
