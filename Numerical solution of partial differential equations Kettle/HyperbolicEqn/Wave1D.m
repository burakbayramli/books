...
% using an explicit central difference method for the 1D wave equation U_tt = c^2*U_xx
x1=0;
...
for i = 1:n
    if x(i) <= 0.8*a
       U_tk_last(i)= 1.25*x(i)/a;

    else
       U_tk_last(i) = 5.*(1-x(i)/a);
    end
end
