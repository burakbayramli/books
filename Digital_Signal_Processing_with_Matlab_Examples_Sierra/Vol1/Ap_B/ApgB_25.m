...
% the signal
[y,fs]=wavread('dog1.wav'); %read wav file

% transform parameters
nft=256; %FFT length
Ny=length(y); %signal length, even
nw=137; %window size, odd
nov=110; %overlapping
...
figure(1)
specgram(y,nft,fs);
title('spectrogram of dog bark')

figure(2)
imagesc(Tc,Fc,20*log10(abs(FT)),[-50 0]); axis xy;
title('Fan-Chirp transform of dog bark')
