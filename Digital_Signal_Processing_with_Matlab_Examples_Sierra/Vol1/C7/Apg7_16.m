...
yc=hilbert(ym); %central signal

%frequency shear
fb=0; fe=40; tt=(1:401)/401;
M=fb+((fe-fb)*tt); %set of modulation frequencies
yx=yc.*exp(-(i*2*pi*M).*tt); %chirp modulation
...
title('Wigner distribution of prolate signal with frequency shear');
