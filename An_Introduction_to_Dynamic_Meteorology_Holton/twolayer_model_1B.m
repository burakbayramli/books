% MATLAB file:  twolayer_model_1B.m
...
% Dynamic Meteorology. This version differs from 1A in that a finite
...
beta = 7.292e-5*cos(pi/4)/6.37e6;   % beta at 45 deg north
...
m = pi/3.e6;                         % meridional wavenumber
K2 = k^2 + m^2;
...
cp = Um - beta*(K2+lam2)/(K2*(K2+2*lam2))+sqrt(del);    % complex zonal phase speed m/s  

% NOTE that x and y are in km  for convenience in graphing
% the factors of 1000 are to convert  to m 
%
t = 0;
psiM = 1.e7*exp(i*k*(x*1.e3-cp*t));
psiT = psiM*(K2-2*lam2)*UT/((cp-Um)*(K2+2*lam2)+beta);
