// IsentropicVortexIC2D.m
// function Q = IsentropicVortexIC2D(xi, yi, ti)
// 2007/06/21
//---------------------------------------------------------
...
//---------------------------------------------------------
void CurvedEuler2D::IsentropicVortexIC2D
(
  const DVec&   xi,   // [in]
  const DVec&   yi,   // [in]
        double  ti,   // [in]
...
//---------------------------------------------------------
{
  // function Q = IsentropicVortexIC2D(xi, yi)
  //
  ...
  static DVec u, v;  DVec xmut, ymvt, rsqr, ex1r, tv1, rho1, p1;
  int Nr = Qo.num_rows(); u.resize(Nr); v.resize(Nr);
  ...
  Qo(All,2) = rho1.dm(u);
  Qo(All,3) = rho1.dm(v);
  Qo(All,4) = p1/gm1 + 0.5*rho1.dm(sqr(u)+sqr(v));
}
